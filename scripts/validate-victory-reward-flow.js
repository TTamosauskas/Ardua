...
for(const token of ["$('phaseQuickMap')?.addEventListener('click'","closeQuickMenu(false)","trigger.click();"]){if(!forkLinks.includes(token))fail('Phase menu Map bridge missing canonical trigger path: '+token)}
const quickMapBlock=forkLinks.slice(forkLinks.indexOf("$('phaseQuickMap')?.addEventListener('click'"),forkLinks.indexOf("$('phaseQuickDiscoveries')?.addEventListener('click'"));
if(!quickMapBlock||quickMapBlock.includes('ARDUA_VICTORY_REWARD')||quickMapBlock.includes('reward?.openMap'))fail('Phase menu Map must not depend on victory reward state');
...
console.log('P1.5 victory reward contract OK: P0 remains canonical; Menu → Mapa is independent of victory reward state and the Map navigation bundle is coherent/cache-busted.');
