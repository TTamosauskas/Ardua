...
for(const token of ["new CustomEvent('ardua:phase-completion-intent'","source:found?.id||'native-engine'"]){if(!completion.includes(token))fail('Completion intent bridge missing: '+token)}
...
if(!index.includes('campaign-phase-completion.js?v=20260911-p1-1'))fail('Completion intent bridge is not cache-busted in index');
const rewardPos=index.indexOf('assets/js/campaign-victory-reward.js'),completionPos=index.indexOf('assets/js/campaign-phase-completion.js'),modalPos=index.indexOf('assets/js/campaign-phase-modal.js');
if(rewardPos<0||completionPos<0||modalPos<0||rewardPos>completionPos||completionPos>modalPos)fail('Victory reward must listen before completion and reuse the later canonical phase preview launcher');
console.log('P1 victory reward contract OK: save/completion stays canonical, reward owns the interstitial, branches defer to the map and unique next phases reuse the canonical launcher.');
