...
const low=['low_mass_formation','he_red','stellar_movement','solar_wind','stellar_ionization','thermal_ionization','stellar_recombination'];
if(JSON.stringify(G.sequences.red)!==JSON.stringify(low))fail('Plasma deve encerrar a trilha de baixa massa');
for(const id of ['solar_wind','stellar_ionization','thermal_ionization','stellar_recombination'])if(G.sequences.mid.includes(id))fail(id+' ainda está na trilha intermediária');
...
if(JSON.stringify(G.prerequisites.thermal_ionization)!==JSON.stringify({allOf:['stellar_ionization']}))fail('Ionização Térmica deve vir após Ionização Estelar');
if(JSON.stringify(G.prerequisites.stellar_recombination)!==JSON.stringify({allOf:['thermal_ionization']}))fail('Recombinação deve vir após Ionização Térmica');
...
for(const id of ['solar_wind','stellar_ionization','thermal_ionization','stellar_recombination']){const at=engine.indexOf(`id:'${id}'`),line=engine.slice(at,engine.indexOf('\n',at));if(at<0||!line.includes("visual:'redDwarf'")||!line.includes('fill:15'))fail(id+' precisa usar núcleo + 2 camadas da Anã Vermelha')}
...
for(const id of ['solar_wind','stellar_ionization','thermal_ionization','stellar_recombination'])if(giant.includes(`'${id}'`))fail('Mapa das gigantes ainda captura fase de plasma: '+id);
if(modal.includes('phase-preview-discoveries')||modal.includes('Descobertas da fase:'))fail('Modal ainda antecipa descobertas');
if(!campaign.includes('version:15')||!campaign.includes("next.activeId='thermal_ionization'")||!campaign.includes("next.activeId='solar_wind'"))fail('Migrações v15/v14/v13 da trilha estelar ausentes');
