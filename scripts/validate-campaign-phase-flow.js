...
for(const token of ["launch.textContent='CONTINUAR'","st.activeId===id||done.has(id)||C.isUnlocked?.(id)","const id=previewId;if(!id||!phaseAccessible(id))return","const id=node.dataset.phase;e.preventDefault();e.stopImmediatePropagation();openPreview(id)","function enginePhaseButton(id)","C.setActive?.(id)","button.click();finishLaunchHandoff()","new MutationObserver(dismissEngineIntro).observe(engineIntro","function shouldDismissEngineIntro(){return map.classList.contains('show')||"]){if(!modal.includes(token))throw new Error('Fluxo de modal de fase perdeu contrato: '+token)}
