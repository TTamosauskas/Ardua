...
const exploration=read('assets/css/campaign-exploration.css');
...
expect(exploration.includes('.phase-node.completed:not(.exploration-hidden)'),'completed phases must retain a distinct travelled-path visual');
expect(exploration.includes('.phase-node.exploration-next.available')&&exploration.includes('.phase-node.exploration-next.current'),'next playable phase must have an explicit available/current visual');
expect(exploration.includes('.phase-node.exploration-preview.locked')&&exploration.includes('pointer-events:none'),'future previews must be visible but unmistakably locked');
