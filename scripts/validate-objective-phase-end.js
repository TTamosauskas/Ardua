...
ok(engine.includes('function activatePhaseCompletionGate(s=phase())'), 'normal engine completion gate is missing');
ok(engine.includes("dom.star.classList.add('phase-ready')")&&engine.includes("$('phaseEndBtn').classList.add('show')"), 'completion gate must darken the board before exposing the final button');
ok(engine.includes('state.locked=true')&&engine.includes('cancelParticleDrag();cancelPrimordialFreeDrag()'), 'completion gate must lock gameplay interactions');
ok(engine.includes('setTimeout(()=>activatePhaseCompletionGate(s),720)'), 'normal engine end-button arming contract changed');
