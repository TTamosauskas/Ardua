...
for(const token of ["function stellarBoardMovementDragAllowed(s=phase()){return atomicMovementAllowed(s)&&s.mode!=='whiteCompact'}","(stellarBoardMovementDragAllowed(s)||fusionSandboxAllowed(s))","if((neigh[p.cell]||[]).some(cell=>{const id=state.board[cell],q=id?state.pieces.get(id):null;return !!stellarBoardAdjacentFusionTarget(p,q,s)}))return true","if(!stellarBoardMovementDragAllowed(s))return false","if(stellarBoardMovementDragAllowed(s))for(const cell of movableEmptyNeighbors"])if(!engine.includes(token))fail('Fusão por drag voltou a depender do desbloqueio de movimento: '+token);
const fusionDragStart=engine.indexOf('function stellarBoardAdjacentFusionTarget'),fusionDragEnd=engine.indexOf('\nfunction stellarBoardDragSourceAvailable',fusionDragStart),fusionDrag=engine.slice(fusionDragStart,fusionDragEnd);
if(fusionDragStart<0||fusionDragEnd<0||fusionDrag.includes("s.mode==='rpProcess'"))fail('rp-process ainda exclui fusão por drag apesar da fusão cumulativa por clique');
if(engine.indexOf("id:'brown'")>engine.indexOf("id:'stellar_movement'")||engine.indexOf("id:'he_red'")>engine.indexOf("id:'stellar_movement'"))fail('Auditoria de drag precoce perdeu Anã Marrom/Anã Vermelha antes do tutorial de movimento');
for(const token of ["s.mode==='reactionExplore'","atlasPairMatches(sp,[source.sym,target.sym])?{atlas:true","if(recipe.atlas){render();","setTimeout(()=>resolveAtlasAttempt(cells),80)"])if(!engine.includes(token))fail('Arraste direto do Atlas perdeu a fusão sem clique prévio: '+token);
...
console.log('Movement/fusion drag parity, campaign return and strict chain knowledge OK.');
