...
O Big Bang usa uma linguagem musical própria para não imitar uma reação nuclear: o toque na singularidade inicia um impacto grave, frases ascendentes sincronizadas às ondas de ejeção e um acorde amplo de resolução durante a expansão. As notas agendadas só tocam enquanto a fase de abertura continua ativa.
...
Notificações com os cabeçalhos `OBJETIVO CIENTÍFICO`, `MARCO` e `ATLAS ATUALIZADO` não desaparecem por tempo: exibem `CONTINUAR` e permanecem visíveis até a confirmação do jogador. Se outra recompensa importante ocorrer enquanto uma dessas mensagens está aberta, ela é mantida em espera e apresentada após a confirmação.
