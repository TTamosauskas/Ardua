 ...
 <main class="campaign-content" id="campaignContent">
  ...
  <div class="campaign-trail" id="campaignTrail">
   ...
   <section class="stellar-grid" data-branch-zone="stellar">
    ...
    <article class="stellar-lane high">${ambientImage('supernova','lane-bg lane-bg-right')}<h2>Alta massa</h2>${flow(G.sequences.high)}${portal('Processo-s fraco',G.sequences.weakS,false,'weak-s')}${flow(G.sequences.collapse)}${structural('Supernova')}<div class="supernova-split">${flow(['nu_f'])}${flow(['gamma_process'])}</div></article>
   </section>
...
function drawLinks(){
 ...
 connectTrail(G.sequences.primordialLeft,'primordial');connectTrail(G.sequences.primordialRight,'primordial');
 addPath(byPhase(tail('primordial_td')),byPhase('primordial_li'),'primordial');addPath(byPhase(tail('primordial_he3d')),byPhase('primordial_li'),'primordial');
 ...
 connectTrail(G.sequences.brown,'sub');connectTrail(G.sequences.red,'low');connectTrail(G.sequences.mid,'mid');connectTrail(G.sequences.high,'high');
 ...
 addPath(byPhase(tail('he_red')),byPhase('white'),'converge',.64);
 ...
 addPath(byPhase(tail('final_collapse')),byPhase('nu_f'),'high',.42);connectIds(expanded(['nu_f']),'high');
 addPath(byPhase(tail('final_collapse')),byPhase('gamma_process'),'high',.42);
 addPath(byPhase(tail('final_collapse')),byPhase('neutron_star'),'compact',.38);connectTrail(G.sequences.remnant,'compact');
 ...
 const binary=map.querySelector('.binary-junction'),kilo=map.querySelector('.kilonova-junction'),rPortal=portalSummary('r');
 addPath(byPhase(tail('neutron_star')),binary,'r',.42);addPath(binary,kilo,'r');addPath(kilo,rPortal,'r');
 ...
 connectTrail(G.sequences.interstellar,'interstellar');connectTrail(G.sequences.decay,'radio');addPath(byPhase(tail('o')),byPhase('spallation_be'),'interstellar',.68);
}
