...
function incomingParticlePoint(){const size=starSize(),margin=Math.max(34,size*.09),edge=Math.floor(Math.random()*4),along=margin+Math.random()*Math.max(1,size-margin*2);if(edge===0)return{x:-margin,y:along};if(edge===1)return{x:size+margin,y:along};if(edge===2)return{x:along,y:-margin};return{x:along,y:size+margin}}
