...
const PHASES=[
 ...
 {id:'coulomb_intro',branch:'Gigante vermelha · nova habilidade',title:'Barreira de Coulomb',meta:'³He + ³He → ⁴He + 2p',new:'He',mode:'fusion',target:3,visual:'redGiant',fill:10,pool:['H'],gravityDelay:143,fusionTempMax:1.2e8,objectiveOnlyProgress:true,menuTag:'COULOMB',endLabel:'PRODUZIR<br>LÍTIO'},
 ...
 {id:'ba',branch:'Segundo pico do processo-s',title:'Bário',meta:'Capturas lentas acumulam matéria na região do segundo pico-s',new:'Ba',mode:'neutron',seed:'Cs',path:['Cs','Ba'],captures:2,target:3,seedCount:4,visual:'agb',fill:31,pool:['He','C','O','Cs','Cs','Cs'],neutronRate:1320,micro:true,fusionTempMax:3.5e8},
 {id:'la',branch:'Microfase · captura de nêutrons',title:'Lantânio',meta:'Ba + n → isótopo instável → β− → La',new:'La',mode:'neutron',seed:'Ba',path:['Ba','La'],captures:1,target:2,seedCount:4,visual:'agb',fill:31,pool:['He','C','O','Ba','Ba','Ba'],neutronRate:1200,micro:true,fusionTempMax:3.5e8},
 {id:'ce',branch:'Microfase · captura de nêutrons',title:'Cério',meta:'La + n → isótopo instável → β− → Ce',new:'Ce',mode:'neutron',seed:'La',path:['La','Ce'],captures:1,target:2,seedCount:4,visual:'agb',fill:31,pool:['He','C','O','La','La','La'],neutronRate:1200,micro:true,fusionTempMax:3.5e8},
 ...
 {id:'pb',branch:'Região terminal do processo-s',title:'Chumbo',meta:'Muitas capturas e β− são agregadas nesta passagem até Pb',new:'Pb',mode:'neutron',seed:'Sm',captures:4,target:3,seedCount:4,visual:'agb',fill:31,pool:['He','C','O','Sm','Sm','Sm'],neutronRate:1400,fusionTempMax:3e8},
 {id:'bi',branch:'Limite do processo-s',title:'Bismuto',meta:'Pb + capturas/β− → Bi · região terminal da rota lenta',new:'Bi',mode:'neutron',seed:'Pb',path:['Pb','Bi'],captures:2,target:2,seedCount:4,visual:'agb',fill:31,pool:['He','C','O','Pb','Pb','Pb'],neutronRate:1450,micro:true,endLabel:'MUDAR PARA<br>PROCESSO-R',fusionTempMax:3.5e8},
 {id:'eu',branch:'Kilonova · início das terras raras',title:'Európio',meta:'Captura rápida de nêutrons · marcador clássico do processo-r',new:'Eu',mode:'neutron',seed:'Sm',captures:3,target:2,seedCount:4,visual:'kilonova',fill:46,pool:['Fe','Ni','Sm','Sm','Sm'],neutronRate:510,rprocess:true},
 ...
 {id:'pt',branch:'Terceiro pico do processo-r',title:'Platina',meta:'Fluxo extremo de nêutrons alcança a região da Platina',new:'Pt',mode:'neutron',seed:'Ir',path:['Ir','Pt'],captures:3,target:2,seedCount:4,visual:'kilonova',fill:46,pool:['Ni','Ir','Ir','Ir'],neutronRate:470,rprocess:true},
 {id:'au',branch:'Captura rápida de nêutrons',title:'Ouro',meta:'Formação de Ouro em material extremamente rico em nêutrons',new:'Au',mode:'neutron',seed:'Pt',path:['Pt','Au'],captures:2,target:2,seedCount:4,visual:'kilonova',fill:46,pool:['Ni','Pt','Pt','Pt'],neutronRate:450,rprocess:true},
 ...
 {id:'th',branch:'Actinídeos',title:'Tório',meta:'Muitas capturas e β− são agregadas até os actinídeos',new:'Th',mode:'neutron',seed:'Tl',captures:5,target:2,seedCount:4,visual:'kilonova',fill:46,pool:['Ni','Tl','Tl','Tl'],neutronRate:420,rprocess:true},
 {id:'u',branch:'Extremo da nucleossíntese',title:'Urânio',meta:'O processo-r alcança um dos núcleos naturais mais pesados',new:'U',mode:'neutron',seed:'Th',path:['Th','U'],captures:4,target:2,seedCount:4,visual:'kilonova',fill:46,pool:['Ni','Th','Th','Th'],neutronRate:400,rprocess:true,endLabel:'ACOMPANHAR<br>O TEMPO'},
 ...
 {id:'accretion',branch:'A estrela recebe nova matéria',title:'Acreção extrema',meta:'Matéria nova chega continuamente pela periferia',new:'Fe',mode:'accretion',target:6,visual:'accretion',fill:12,feedCap:14,endEvent:'postTransition',endLabel:'ATINGIR<br>O LIMITE',menuTag:'ACREÇÃO'},
...
async function atlasCompleteChannel(sp,cells,ids,{endothermic=false}={}){
 const target=[...cells].sort((a,b)=>(coords[a]?.ring??99)-(coords[b]?.ring??99))[0],t=pos(coords[target]);
 if(sp.category==='competing')await atlasShowBranches(sp,t.x,t.y);
 if(endothermic){atlasEnergyInflow(t.x,t.y);await wait(520)}
 cells.forEach(c=>state.board[c]=null);ids.forEach(id=>state.pieces.delete(id));
 const product=createPiece(sp.mainSym,target,false,{massNumber:sp.mainA});product.x=t.x;product.y=t.y;product.newborn=true;focusPieceInfo(product);state.created[sp.mainSym]=(state.created[sp.mainSym]||0)+1;state.discovered.add(sp.mainSym);burst(t.x,t.y);
 if(sp.channel==='gamma')await emitGamma(t.x,t.y);else await atlasCreateSecondary(sp,target,t.x,t.y);
 state.atlasProgress++;recordFlow(1);state.selected=[];delete state.atlasBarrierPassed[atlasPairKey(ids)];tone(560+Math.min(260,(E[sp.mainSym]?.n||1)*6),.10,'triangle',.038);
 setTimeout(()=>{const q=state.pieces.get(product.id);if(q){q.newborn=false;renderPieces()}},360);await afterNuclearAction({advanceRound:true,replenish:true,protectedPieceIds:[product.id]});ensureAtlasOpportunity();render();checkComplete();
}
async function atlasCreateFragmentCompound(sp,cells,ids){
 const target=[...cells].sort((a,b)=>(coords[a]?.ring??99)-(coords[b]?.ring??99))[0],t=pos(coords[target]);
 cells.forEach(c=>state.board[c]=null);ids.forEach(id=>state.pieces.delete(id));
 const compound=createPiece(sp.compound,target,false,{massNumber:sp.compoundA});compound.x=t.x;compound.y=t.y;compound.atlasCompound=true;compound.atlasSpecId=sp.id;compound.atlasLabel=`${infoSymbolFor(sp.compound)}*`;armPieceInstability(compound,{rounds:sp.rounds,mode:'atlasFragment',label:sp.label},state.nuclearRound+1);focusPieceInfo(compound);state.selected=[];delete state.atlasBarrierPassed[atlasPairKey(ids)];burst(t.x,t.y);tone(390,.12,'sawtooth',.03);renderPieces();
 await afterNuclearAction({advanceRound:true,replenish:true,protectedPieceIds:[compound.id]});ensureAtlasOpportunity();render();
}
...
function protonCaptureAvailable(s=phase()){
 ...
 return s.mode==='protonCapture'||state.protonCaptureUnlocked||(intro!==undefined&&state.phaseIndex>=intro);
}
...
function protonCaptureRoute(targetOrSym,s=phase()){
 const target=typeof targetOrSym==='string'?{sym:targetOrSym,massNumber:null}:targetOrSym;if(!target)return null;const r=PROTON_CAPTURES[target.sym];if(!r)return null;
...
async function attemptProtonCapture(cell,protonId){
 ...
 const success=!!route&&!blocked;
 if(!success){
   state.coulombRepulsions++;p.coulombDeflect=true;const side=(target.id%2?1:-1),px=-dy/len,py=dx/len;p.x=Math.max(24,Math.min(starSize()-24,ox+px*side*(30+Math.min(34,z))));p.y=Math.max(24,Math.min(starSize()-24,oy+py*side*(30+Math.min(34,z))));renderPrimordialParticles();tone(150+Math.min(170,z*4),.10,'sawtooth',.026);vibrate(z>=10?[7,18,7]:6);await wait(360);p.reacting=false;p.coulombDeflect=false;state.primordialSelected=protonId;state.selected=[];state.locked=false;render();return;
 }
 p.tunneling=true;emitTunnelGhosts(p.x,p.y,target.x,target.y);p.x=target.x;p.y=target.y;renderPrimordialParticles();tone(760,.08,'triangle',.038);await wait(300);state.primordialParticles.delete(protonId);target.sym=route.out;target.massNumber=route.productMass||null;target.captures=0;target.newborn=true;clearPieceInstability(target);if(route.decay)armPieceInstability(target,route.decay,state.nuclearRound+1);else if(E[target.sym]?.unstable)armIntrinsicInstability(target,state.nuclearRound+1);
 const productKey=`${target.massNumber||''}${target.sym}${pieceIsUnstable(target)?'*':''}`;state.protonCaptureProducts[productKey]=(state.protonCaptureProducts[productKey]||0)+1;burst(target.x,target.y);renderPieces();if(pieceIsUnstable(target)){captureTag(target.x,target.y,route.decay?.mode==='returnProton'?'estado não ligado':'núcleo instável');tone(620,.10,'triangle',.032)}else captureTag(target.x,target.y,'p,γ');
 if(route.gamma!==false)await emitGamma(target.x,target.y);if(route.countsCapture!==false){state.protonCaptures++;state.discovered.add(route.out);if(s.mode==='protonCapture')recordFlow(1)}
 setTimeout(()=>{const q=state.pieces.get(target.id);if(q){q.newborn=false;renderPieces()}},360);await afterNuclearAction({advanceRound:true,protectedPieceIds:[target.id]});state.protonCaptureAttempts[key]=0;ensureProtonCaptureFuel(s.mode==='protonCapture'?3:2);state.selected=[];state.locked=false;render();checkComplete();
}
...
function pieceDisplaySymbol(p){if(p?.atlasLabel)return p.atlasLabel;const e=E[p.sym],base=e?.symbol||p.sym;if(p.matterState==='atom'){const q=pieceCharge(p);return q>0?`${p.sym}${q===1?'⁺':superNum(q)+'⁺'}`:p.sym}if(['D','T','He3','Be7'].includes(p.sym))return base;if(p.massNumber&&['H','He','Li'].includes(p.sym))return `${superNum(p.massNumber)}${p.sym}`;return base}
...
function renderPieces(){
 ...
 state.pieces.forEach((p,id)=>{
  ...
  if(!el){
   ...
   el.addEventListener('pointerdown',ev=>{if(['neutronize','neutron','guidedDecay','decayGarden'].includes(phase().mode)||isPostAtomMode()){ev.stopPropagation();beginCrushHold(id,el,ev)}});
   ...
   if(!window.PointerEvent){let touchHeld=false;el.addEventListener('touchstart',ev=>{if(['neutronize','neutron','guidedDecay','decayGarden'].includes(phase().mode)||isPostAtomMode()){touchHeld=true;beginCrushHold(id,el,ev)}},{passive:true});el.addEventListener('touchend',()=>{const wasHolding=state.crushId===id;cancelCrushHold(id);if(touchHeld&&wasHolding&&!(state.suppressTapId===id&&performance.now()<state.suppressTapUntil))setTimeout(()=>tapAtom(id),0);touchHeld=false},{passive:true});el.addEventListener('touchcancel',()=>{touchHeld=false;cancelCrushHold(id)},{passive:true})}
  ...
  el.className='atom'+matterClass+(p.sym==='Plus'?' proton-piece':'')+(selected?' selected':'')+(partner&&!selected?' candidate':'')+(decayReady?' decay-ready':'')+(pieceIsUnstable(p)?' unstable':'')+(p.longRadioactive?' long-radioactive':'')+(p.radioactiveReady?' radioactive-proof':'')+(p.compacted?' compacted':'')+(p.atlasCompound?' atlas-compound':'')+(p.atlasRebound?' atlas-rebound':'')+(p.newborn?' newborn':'');el.style.left=p.x+'px';el.style.top=p.y+'px';if(primordial&&p.free){el.style.setProperty('--floatDelay',`${-((id%19)*.17)}s`)}else el.style.removeProperty('--floatDelay');existing.delete(id)
...
async function reactPrimordialParticlePair(r,a,b){
 if(state.locked||!a||!b||a.reacting||b.reacting)return;state.locked=true;state.primordialSelected=null;const x=(a.x+b.x)/2,y=(a.y+b.y)/2;a.reacting=true;b.reacting=true;a.x=x;a.y=y;b.x=x;b.y=y;renderPrimordialParticles();await wait(260);state.primordialParticles.delete(a.id);state.primordialParticles.delete(b.id);const out=createFreePiece(r.out,x,y,{massNumber:r.mass,longRadioactive:!!r.longRadioactive});out.newborn=true;focusPieceInfo(out);state.created[r.out]=(state.created[r.out]||0)+1;state.discovered.add(r.out);recordFlow(r.out===phase().new?2:1);burst(x,y);render();await handleReactionEmissions(r,out);await afterNuclearAction({advanceRound:true});setTimeout(()=>{const q=state.pieces.get(out.id);if(q){q.newborn=false;renderPieces()}},300);state.locked=false;render();checkComplete()
}
async function reactPrimordialMixed(r,piece,particle){
 if(state.locked||!piece||!particle||particle.reacting)return;state.locked=true;state.freeSelected=[];state.primordialSelected=null;const x=piece.x,y=piece.y;particle.reacting=true;particle.x=x;particle.y=y;renderPrimordialParticles();await wait(220);state.primordialParticles.delete(particle.id);state.pieces.delete(piece.id);const out=createFreePiece(r.out,x,y,{massNumber:r.mass,longRadioactive:!!r.longRadioactive});out.newborn=true;focusPieceInfo(out);state.created[r.out]=(state.created[r.out]||0)+1;state.discovered.add(r.out);recordFlow(r.out===phase().new?2:1);burst(x,y);render();await handleReactionEmissions(r,out);await afterNuclearAction({advanceRound:true});setTimeout(()=>{const q=state.pieces.get(out.id);if(q){q.newborn=false;renderPieces()}},300);state.locked=false;render();checkComplete()
}
async function recombineHydrogenParticles(a,b){
 if(state.locked||a.reacting||b.reacting)return;const p=a.kind==='p'?a:b,e=a.kind==='e'?a:b;if(!p||!e)return;state.locked=true;state.primordialSelected=null;p.reacting=true;e.reacting=true;const x=(p.x+e.x)/2,y=(p.y+e.y)/2;p.x=x;p.y=y;e.x=x;e.y=y;renderPrimordialParticles();await wait(260);state.primordialParticles.delete(p.id);state.primordialParticles.delete(e.id);const h=createFreePiece('H',x,y,{matterState:'atom',boundElectrons:1,massNumber:1});h.newborn=true;focusPieceInfo(h);state.created.H=(state.created.H||0)+1;state.discovered.add('H');recordFlow(1);burst(x,y);render();await emitGamma(x,y);await afterNuclearAction({advanceRound:true});setTimeout(()=>{const q=state.pieces.get(h.id);if(q){q.newborn=false;renderPieces()}},300);state.locked=false;render();checkComplete()
}
async function bindElectronToPiece(piece,electron){
 if(state.locked||!pieceCanBindElectron(piece)||!electron||electron.kind!=='e')return;state.locked=true;state.freeSelected=[];state.primordialSelected=null;electron.reacting=true;electron.x=piece.x;electron.y=piece.y;renderPrimordialParticles();await wait(220);state.primordialParticles.delete(electron.id);piece.boundElectrons=Math.min(E[piece.sym].n,Number(piece.boundElectrons||0)+1);piece.matterState='atom';piece.newborn=true;focusPieceInfo(piece);recordFlow(1);if(pieceCharge(piece)===0){state.created[piece.sym]=(state.created[piece.sym]||0)+1;state.discovered.add(piece.sym)}burst(piece.x,piece.y);render();await emitGamma(piece.x,piece.y);await afterNuclearAction({advanceRound:true});setTimeout(()=>{const q=state.pieces.get(piece.id);if(q){q.newborn=false;renderPieces()}},300);state.locked=false;render();checkComplete()
}
...
async function fuseFree(r,ids){
 if(state.locked)return;const parts=ids.map(id=>state.pieces.get(id)).filter(Boolean);if(parts.length!==r.pieces.length)return;state.locked=true;const x=parts.reduce((n,p)=>n+p.x,0)/parts.length,y=parts.reduce((n,p)=>n+p.y,0)/parts.length;parts.forEach(p=>{p.x=x;p.y=y});renderPieces();await wait(150);ids.forEach(id=>state.pieces.delete(id));state.freeSelected=[];const out=createFreePiece(r.out,x,y,{massNumber:r.mass,longRadioactive:!!r.longRadioactive});out.newborn=true;focusPieceInfo(out);state.created[r.out]=(state.created[r.out]||0)+1;state.discovered.add(r.out);recordFlow(r.out===phase().new?2:1);burst(x,y);render();await handleReactionEmissions(r,out);await afterNuclearAction({advanceRound:true});setTimeout(()=>{const q=state.pieces.get(out.id);if(q){q.newborn=false;renderPieces()}},300);state.locked=false;render();checkComplete()
}
...
async function fireCosmicRay(rayId,targetId){
 if(state.locked)return;const s=phase(),ray=state.cosmicRays.get(rayId),target=state.pieces.get(targetId),targets=particleTargets(s);if(!ray||!target||!targets.includes(target.sym))return;
 state.locked=true;state.selectedCosmic=null;const el=dom.cosmic?.querySelector(`[data-id="${rayId}"]`);if(el){el.classList.add('firing');el.style.transition='left .38s cubic-bezier(.14,.78,.2,1),top .38s cubic-bezier(.14,.78,.2,1),transform .38s ease';el.style.left=target.x+'px';el.style.top=target.y+'px';el.style.transform='translate(-50%,-50%) scale(.55)'}tone(s.mode==='neutrino'?920:s.mode==='gamma'?820:720,.12,s.mode==='gamma'?'triangle':'sawtooth',.035);await wait(380);
 const {x,y,cell}=target,source=target.sym,out=ray.product;state.cosmicRays.delete(rayId);
 if(s.mode==='gamma'&&s.isotopeMode){
   target.massNumber=Math.max(E[target.sym].n+1,(target.massNumber|| (target.sym==='Mo'?100:104))-1);target.newborn=true;state.created[s.new]=(state.created[s.new]||0)+1;recordFlow(3);spallFragments(x,y);burst(x,y);captureTag(x,y,'γ,n');tone(760,.12,'triangle',.045);vibrate([10,14,10]);setTimeout(()=>{const q=state.pieces.get(target.id);if(q){q.newborn=false;renderPieces()}},340);announce('γ-PROCESSO',`${target.sym} · A=${target.massNumber}`,'O fóton removeu um nêutron; o elemento permaneceu o mesmo, mas o isótopo ficou mais proton-rich.');await afterNuclearAction();state.locked=false;render();checkComplete();if(!state.phaseDone)setTimeout(spawnCosmicRay,120);return
 }if(state.board[cell]===targetId)state.board[cell]=null;state.pieces.delete(targetId);const product=createPiece(out,cell,false);product.x=x;product.y=y;product.newborn=true;focusPieceInfo(product);state.created[out]=(state.created[out]||0)+1;state.discovered.add(out);if(s.mode==='spallation'){if(out===s.new)recordFlow(1)}else recordFlow(out===s.new?3:1);spallFragments(x,y);burst(x,y);tone(430+E[out].n*12,.12,'triangle',.05);vibrate([12,18,14]);setTimeout(()=>{const q=state.pieces.get(product.id);if(q){q.newborn=false;renderPieces()}},360);
 if(!triggerPhaseMilestone()){const tag=s.mode==='neutrino'?'ν-PROCESSO':s.mode==='gamma'?'γ-PROCESSO':'ESPALAÇÃO';announce(tag,`${E[out].name.toUpperCase()} FORMADO`,`${state.created[out]||0}/${s.target}`)}await afterNuclearAction();state.locked=false;render();checkComplete();if(!state.phaseDone)setTimeout(spawnCosmicRay,120)
}
...
function currentProgress(){const s=phase();if(s.mode==='opening')return 0;if(s.mode==='reactionExplore')return Math.min(100,(state.atlasProgress/Math.max(1,s.target||1))*100);if(isPrimordial(s))return Math.min(100,(primordialGoalCount(s)/Math.max(1,s.target))*100);if(s.mode==='protonCapture')return Math.min(100,(state.protonCaptures/Math.max(1,s.target||1))*100);if(s.id==='brown')return Math.min(100,((state.created.He3||0)/brownBurnLimit())*100);if(s.mode==='whiteCompact'){const info=whiteCounts(s);return Math.min(100,((Math.min(info.c,info.targetC)+Math.min(info.o,info.targetO))/Math.max(1,info.targetC+info.targetO))*100)}return Math.min(100,(state.flow/Math.max(1,s.flowTarget||1))*100)}
function recordFlow(points=1){
 const s=phase();if(s.mode==='opening'||state.phaseDone)return;
 const before=state.flow;state.flow+=Math.max(0,points);
 if(s.id==='brown')return;
 const target=Math.max(1,s.flowTarget||1),marks=[[.25,'25'],[.5,'50'],[.75,'75']];
 for(const [ratio,key] of marks){if(before<target*ratio&&state.flow>=target*ratio&&!state.flowMilestones.has(key)){state.flowMilestones.add(key);setTimeout(()=>{if(phase()!==s||state.phaseDone)return;announce('ATIVIDADE DA FASE',`${key}% DO AMBIENTE EXPLORADO`,key==='50'?'Você já domina o gesto desta fase. Continue no seu ritmo.':'Reações compatíveis também contam para esta atividade.');},240)}}
}
...
function conciseRecipeLine(s=phase()){
 ...
 if(s.mode==='blackhole')return 'matéria → horizonte';
 if(s.mode==='fusion'){
...
function updateHUD(){
 ...
 const p=currentProgress(),flowTarget=Math.max(0,s.flowTarget||0),flowShown=Math.min(state.flow,flowTarget);
 $('stageProgress').style.width=p+'%';
 if(s.mode==='reactionExplore')$('stageProgressText').textContent=`${state.atlasProgress}/${s.target}`;
 ...
 $('phaseEndBtn').innerHTML=s.endLabel||'ESPALHAR<br>POEIRA ESTELAR';updateObjective();applyVisual();renderInfoPanel()
}
function updateObjective(){
 ...
 if(s.mode==='blackhole'){$('goalText').textContent=`Matéria restante — ${state.pieces.size}`;setFormula(conciseRecipeLine(s));return}
...
function applyVisual(){const s=phase(),v=s.visual,root=document.documentElement,styles={nebula:['#fff3bc','#ffae5f','#4d2e82','rgba(255,126,72,.27)',.96],brownDwarf:['#ffe0a6','#9f532d','#351d31','rgba(169,86,47,.30)',.72],redDwarf:['#fff0ca','#ff7757','#6c2231','rgba(255,84,65,.38)',.84],orangeDwarf:['#fff6ce','#ffad56','#8c3e28','rgba(255,145,67,.42)',.91],yellowDwarf:['#fffbe0','#ffd66e','#a25228','rgba(255,199,81,.48)',1],whiteDwarf:['#ffffff','#dff2ff','#819bc2','rgba(210,237,255,.62)',.72],solar:['#fff9ce','#ffd06c','#9a4e28','rgba(255,192,83,.4)',1],redGiant:['#fff0b0','#ff7e48','#771d28','rgba(255,80,46,.48)',1.06],agb:['#ffe7a3','#f06d42','#6c2148','rgba(255,88,55,.42)',1.04],massive:['#eff8ff','#93c8ff','#3340a1','rgba(99,160,255,.48)',1.02],supergiant:['#fff','#b1d6ff','#4b4ac7','rgba(111,158,255,.58)',1.07],advanced:['#fffbd7','#ffb64f','#8d2843','rgba(255,132,67,.62)',1.08],ironCore:['#fff','#ff975a','#521722','rgba(255,72,47,.65)',1.08],kilonova:['#fff','#a6d8ff','#9e54d8','rgba(177,110,255,.6)',.96],interstellar:['#dfeaff','#6686c9','#18254d','rgba(85,129,215,.28)',.96],neutronStar:['#eaf9ff','#78baff','#1a2d80','rgba(104,178,255,.55)',.90],pulsar:['#fff','#89c9ff','#203ca3','rgba(128,199,255,.68)',.90],accretion:['#fff6cf','#ffb24e','#491d42','rgba(255,142,61,.58)',.92],collapseFinal:['#fff','#ff8b62','#270b18','rgba(255,74,57,.66)',.88],blackHole:['#1b2543','#070914','#000','rgba(89,126,255,.20)',.90]};const a=styles[v]||styles.solar,prebang=s.mode==='opening'&&!state.bigBangColorized;document.body.classList.toggle('prebang',prebang);document.body.classList.toggle('bigbang-phase',s.mode==='opening');const spaceColors={bigBang:'#e10b17',primordialH:'#9f0814',primordialHe:'#4e0710',primordialLi:'#160307'};root.style.setProperty('--spaceBg',prebang?'#fff':(spaceColors[v]||'#000'));root.style.setProperty('--primordialGlow',prebang?'rgba(255,255,255,0)':v==='bigBang'?'rgba(255,210,130,.58)':v==='primordialH'?'rgba(255,80,40,.36)':v==='primordialHe'?'rgba(201,41,31,.28)':'rgba(112,24,29,.20)');dom.star.classList.toggle('primordial-mode',isPrimordial(s));dom.star.classList.toggle('spallation-mode',['spallation','neutrino','gamma'].includes(s.mode));dom.star.classList.toggle('decay-mode',s.mode==='guidedDecay');dom.star.classList.toggle('electron-network',s.mode==='guidedDecay'||s.mode==='whiteCompact');dom.star.classList.toggle('white-electron-network',s.mode==='whiteCompact');dom.star.classList.toggle('white-structure',s.mode==='whiteCompact');dom.star.classList.toggle('cumulative-shells',s.mode!=='fusion'&&s.mode!=='whiteCompact'&&fusionSandboxAllowed(s)&&stratificationStrength(s)>0);root.style.setProperty('--starA',a[0]);root.style.setProperty('--starB',a[1]);root.style.setProperty('--starC',a[2]);
  ...
  root.style.setProperty('--starGlow',glow);root.style.setProperty('--starScale',1);const intensity=isPrimordial(s)?0:visualIntensity();root.style.setProperty('--activityGlow',(intensity/100*.34).toFixed(2));root.style.setProperty('--activityPulseTime',`${Math.max(.72,2.7-intensity*.018)}s`);const redIndex=phaseIndexById.get('he_red')??1,stellarEligible=!isPrimordial(s)&&!isPostMode(s)&&v!=='brownDwarf'&&v!=='nebula',persistentLit=stellarEligible&&(state.phaseIndex>redIndex||state.ignited),active=phaseMilestoneReached(s);dom.star.classList.toggle('ignited',persistentLit);dom.star.classList.toggle('phase-active',active);dom.star.classList.toggle('neutron-active',active&&s.mode==='neutron');dom.star.classList.toggle('critical',v==='ironCore'||v==='kilonova'||s.mode==='collapseFinal');dom.star.classList.toggle('remnant-mode',isPostMode(s));dom.star.classList.toggle('blackhole-mode',s.mode==='blackhole');dom.star.classList.toggle('stability-mode',s.mode==='collapseFinal');dom.star.classList.toggle('post-active',isPostMode(s)&&active);renderRemnant()}
function renderRemnant(){
 const s=phase(),layer=dom.remnant;if(!layer)return;
 layer.className='remnant-layer';
 if(!isPostMode(s)){layer.style.removeProperty('--beamPeriod');return}
 layer.classList.add('show');
 if(s.mode==='remnant')layer.classList.add('neutron-star');
 if(s.mode==='pulsar')layer.classList.add('pulsar');
 if(s.mode==='accretion')layer.classList.add('accretion');
...
function beginCrushHold(id,el,ev){
  ...
  if(state.crushTimer)return;if(isPostAtomMode(s)&&state.postHoldLearned)return;
  ...
  state.crushTimer=setTimeout(()=>s.mode==='decayGarden'?completeGardenDecayHold(id):s.mode==='guidedDecay'||(s.id==='co'&&p.radioactiveReady)?completeGuidedDecay(id):isPostAtomMode()?completePostAbsorb(id):completeCrush(id),1000);
}
...
function closeEventTooltip(){
 ...
 if(isPrimordial()&&phase().mode!=='opening')startPrimordialDrift();
...
async function ejectAlphaHelium(p){
 ...
 if(cell===undefined)return null;const he=createPiece('He',cell,false);he.x=p.x;he.y=p.y;he.newborn=true;requestAnimationFrame(()=>{const q=pos(coords[cell]);he.x=q.x;he.y=q.y;renderPieces()});setTimeout(()=>{const q=state.pieces.get(he.id);if(q){q.newborn=false;renderPieces()}},430);await wait(440);return he;
}
...
async function completePostAbsorb(id){
 ...
 if(el)el.classList.remove('crush-hold');
 if(!isPostAtomMode(s)||state.locked||state.phaseDone||!p)return;
...
async function decayBe8(piece){
 ...
 const firstLesson=!state.productLessons.has('helium'),origin=piece.cell,at={x:piece.x,y:piece.y};if(firstLesson)await teachProductOnce('helium',at.x,at.y);state.board[origin]=null;state.pieces.delete(piece.id);
 const first=createPiece('He',origin,false);first.x=at.x;first.y=at.y;
 ...
 if(secondCell!==undefined){const second=createPiece('He',secondCell,false);second.x=at.x;second.y=at.y;requestAnimationFrame(()=>{const q=pos(coords[secondCell]);second.x=q.x;second.y=q.y;renderPieces()})}
...
function ensureOpportunity(){
  ...
  if(s.mode==='neutron'){
    if(s.chainRebuild){
      // Nessas fases os reagentes históricos são pistas finitas: nunca recrie O, C,
...
function neutronTransitionFor(p,s=phase()){if(!p||s.mode!=='neutron')return null;return currentNeutronTransition(p.sym,s)||learnedNeutronTransitions(s).slice().reverse().find(tr=>tr.from===p.sym)||null}
...
function captureTag(x,y,text){return}
...
function checkComplete(){
 ...
 state.readyToAdvance=true;state.selected=[];if(s.id==='brown')state.locked=true;save();$('phaseEndBtn').classList.add('show');dom.star.classList.add('critical');
 ...
 else if(s.mode==='blackhole')announce('OBJETIVO CONCLUÍDO','HORIZONTE ALIMENTADO','O espaço local foi limpo; conclua o ciclo quando quiser.');
...
async function finishCampaign(){
 if(!state.phaseDone)return;$('phaseEndBtn').classList.remove('show');state.locked=true;dom.star.classList.add('phase-active');announce('CICLO CÓSMICO','CONCLUÍDO','O Buraco Negro permanece como remanescente final desta jornada.');tone(72,.75,'sine',.055);vibrate([22,40,32,55]);toast('Ciclo cósmico concluído · use o Menu para revisitar as fases.');
}
...
function modalPrimaryLine(s=phase()){
 ...
 if(s.mode==='blackhole')return 'Colapso extremo → Buraco Negro';
 if(s.mode==='fusion'){
...
function modalSecondaryLine(s=phase()){
 ...
 if(s.mode==='blackhole')return 'Faça a matéria atravessar o horizonte';
...
function phaseIntroTitle(s=phase(),base={}){
 const special={primordial_d:'PRIMEIRO NÚCLEO',primordial_t:'TRÍTIO',primordial_he3:'HÉLIO-3',primordial_he3d:'HÉLIO-4',primordial_td:'HÉLIO-4',primordial_li:'LÍTIO-7',atomic_he:'HÉLIO',atomic_h:'HIDROGÊNIO',atomic_li:'LÍTIO',coulomb_intro:'BARREIRA DE COULOMB',stellar_li:'LÍTIO ESTELAR'};
...
function phaseFamily(p){
 ...
 if(['brown','he_red','he_orange','he_yellow','coulomb_intro','stellar_li','fragile','c','n','o'].includes(p.id))return 'Estrelas de baixa e média massa';
