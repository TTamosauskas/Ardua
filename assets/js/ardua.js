...
const PHASES=[
 ...
 {id:'first_atomic_bonds',branch:'Química primordial · primeiras ligações',title:'Primeiras Ligações Atômicas',meta:'He + H → HeH⁺',new:'HeH+',mode:'primordialMolecule',target:4,visual:'primordialHe',primordial:true,endLabel:'CRIAR<br>GÁS PRIMORDIAL',menuTag:'HeH⁺'},
 {id:'first_nebulae',branch:'Química primordial · primeiras nebulosas',title:'Primeiras Nebulosas',meta:'HeH⁺ + H → H₂',new:'H2',mode:'primordialMolecule',target:4,visual:'primordialH',primordial:true,endLabel:'FORMAR<br>PRIMEIRA GERAÇÃO',menuTag:'H₂'},
 ...
 {id:'low_mass_formation',branch:'Nascimento estelar · baixa massa',title:'Formação da Estrela de Baixa Massa',meta:'Reúna 9 duplas de H; o último H completa automaticamente a vaga final',new:'H',mode:'stellarFormation',formationLayers:2,target:19,visual:'nebula',fill:0,endEvent:'stellarBirth',endLabel:'FORMAR<br>ANÃ VERMELHA',menuTag:'19 H',durationClass:'quick'},
 ...
 {id:'stellar_movement',branch:'Nova estrela · baixa massa',title:'Movimentação Estelar',meta:'Leve o Hélio até o núcleo estelar.',new:'He',mode:'movementTutorial',target:1,flowTarget:0,visual:'redDwarf',fill:15,pool:['H'],gravityDelay:175,menuTag:'MOVIMENTO',endLabel:'ESPALHAR<br>POEIRA ESTELAR'},
 {id:'solar_wind',branch:'Coroa estelar · plasma e escape',title:'Vento Solar',meta:'H + e⁻ → p⁺ + 2e⁻',new:'H',mode:'stellarIonization',target:3,visual:'redDwarf',fill:15,pool:['H','H','H','H','H','He'],ionizationSpecies:['H'],solarWindTutorial:true,menuTag:'VENTO SOLAR',endLabel:'ESPALHAR<br>POEIRA ESTELAR'},
{id:'stellar_ionization',branch:'Coroa estelar · ionização',title:'Ionização Estelar',meta:'Átomo + e⁻ → Íon⁺ + 2e⁻',new:'H',mode:'stellarIonization',target:3,visual:'redDwarf',fill:15,pool:['H','H','H','H','H','He'],ionizationSpecies:['H','He','Li'],menuTag:'ÍON +1',endLabel:'ESPALHAR<br>POEIRA ESTELAR'},
{id:'stellar_recombination',branch:'Coroa estelar · recombinação',title:'Recombinação Estelar',meta:'Íon⁺ + e⁻ → Átomo + γ',new:'H',mode:'stellarRecombination',target:3,visual:'redDwarf',fill:15,pool:['H','H','H','H','H','He'],recombinationSpecies:['H','He','Li'],menuTag:'RECOMBINAÇÃO',endLabel:'ESPALHAR<br>POEIRA ESTELAR'},
 {id:'intermediate_mass_formation',branch:'Nascimento estelar · massa intermediária',title:'Formação da Estrela de Massa Intermediária',meta:'Reúna 18 duplas de H; o último H completa automaticamente a vaga final',new:'H',mode:'stellarFormation',formationLayers:3,target:37,visual:'nebula',fill:0,endEvent:'stellarBirth',endLabel:'FORMAR<br>ANÃ LARANJA',menuTag:'37 H',durationClass:'short'},
...
{id:'coulomb_intro',branch:'Gigante vermelha · nova habilidade',title:'Barreira de Coulomb',meta:'³He + ³He → ⁴He + 2p',new:'He',mode:'fusion',target:3,visual:'redGiant',fill:28,pool:['H','H','H','H','H','He'],gravityDelay:143,fusionTempMax:1.2e8,objectiveOnlyProgress:true,menuTag:'COULOMB',endLabel:'APRENDER<br>CONVECÇÃO'},
...
{id:'coronal_jets',branch:'Gigante vermelha · atividade magnética',title:'Jatos Coronais',meta:'Convecção + íon superficial → reconexão magnética → jato coronal',new:'He',mode:'convection',target:2,visual:'redGiant',fill:34,pool:['H','H','He','He3'],gravityDelay:142,fusionTempMax:1.2e8,reuseFusion:true,objectiveOnlyProgress:true,coronalJetTutorial:true,menuTag:'JATOS CORONAIS',endLabel:'ESPALHAR<br>POEIRA ESTELAR'},
 ...
 {id:'giant_formation',branch:'Nascimento estelar · bifurcação das gigantes',title:'Formação das Gigantes',meta:'Reúna 30 duplas de H; o último H completa automaticamente a vaga final',new:'H',mode:'stellarFormation',formationLayers:4,target:61,visual:'nebula',fill:0,endEvent:'stellarBirth',endLabel:'ESCOLHER<br>GIGANTE',menuTag:'61 H',durationClass:'long'},
 ...
 {id:'high_mass_formation',branch:'Nascimento estelar · alta massa',title:'Formação da Estrela de Alta Massa',meta:'Reúna 45 duplas de H; o último H completa automaticamente a vaga final',new:'H',mode:'stellarFormation',formationLayers:5,target:91,visual:'nebula',fill:0,endEvent:'stellarBirth',endLabel:'INICIAR<br>QUEIMA DE CARBONO',menuTag:'91 H',durationClass:'epic'},
...
function configurePhaseDurationBudget(phases){
 ...
 setClass(['atomic_li','first_atomic_bonds','first_nebulae','brown','he_red','he_orange','he_yellow','solar_wind','stellar_ionization','stellar_recombination','coulomb_intro','stellar_convection'],'short');
...
function configureNeutronGameplay(phases){
 ...
 // Tempo β em rodadas é uma escala relativa da rota/isótopo representado, não uma meia-vida literal do elemento.
...
const PHASE_FACTS={
 ...
 "first_nebulae":"O H₂ tornou-se um refrigerante decisivo do gás primordial. A rede real passa por intermediários como H₂⁺ e H⁻; o jogo resume essa química em uma transferência direta para mostrar o nascimento do gás molecular.",
 ...
 "solar_wind":"A coroa de uma estrela como o Sol é um plasma extremamente quente. O vento solar é o fluxo de partículas desse plasma que escapa continuamente para o espaço.",
...
function phaseRadius(s=phase()){
  ...
  if(s.mode==='stellarFormation')return stellarFormationSpec(s).radius;
  if(s.id==='brown'||s.visual==='brownDwarf')return 1;
  if(s.id==='he_red'||s.visual==='redDwarf')return 2;
  if(s.id==='he_orange'||s.id==='he_yellow'||s.visual==='yellowDwarf'||s.mode==='whiteCompact'||s.visual==='whiteDwarf')return 3;
...
function phaseGeometry(s=phase()){
  ...
  if(s.mode==='stellarFormation'){const r=stellarFormationSpec(s).radius;return{r,factor:r>=4?.995:r===3?.94:r===2?.90:.78,max:r>=4?600:r===3?540:r===2?470:360}}
...
function desiredFill(){
  ...
  if(s.mode==='stellarFormation')return 0;
...
function renderDiscoveryAtlas(){
 ...
 for(const entry of DISCOVERY_ATLAS){const open=discoveryUnlocked(entry);if(!open)continue;if(entry.group!==group){group=entry.group;const h=document.createElement('div');h.className='discovery-group';h.textContent=group;host.appendChild(h)}const b=document.createElement('button');b.type='button';b.className='discovery-card'+(open?' unlocked':' locked');const glyph=entry.kind==='element'?(E[entry.sym]?.symbol||entry.sym):(entry.glyph||'·');b.innerHTML=`<span class="discovery-glyph">${open?glyph:'?'}</span><span><strong>${open?entry.title:'Não descoberto'}</strong><small>${open?entry.group:'Continue explorando a campanha'}</small></span>`;if(open)b.addEventListener('click',()=>{if(detail)detail.innerHTML=`<strong>${entry.title}</strong><span>${entry.group}</span><p>${entry.text}</p>`});host.appendChild(b)}
}
...
function clearPrimordialParticles(){stopPrimordialDrift();cancelParticleDrag();cancelPrimordialFreeDrag();state.primordialParticles.clear();state.primordialSelected=null;if(dom.primordial)dom.primordial.innerHTML=''}
...
function primordialParticleDropTarget(p){
 const s=phase();if(!p||!isPrimordial(s)||s.mode==='opening'||p.reacting)return null;let best=null,bestDist=Infinity;
 for(const q of state.primordialParticles.values()){
  if(q.id===p.id||q.reacting||q.dragging)continue;
  const pair=primordialParticlePairReaction([p.kind,q.kind]),recombine=atomicRecombinationLearned('H')&&same([p.kind,q.kind],['p','e']);
  if(!pair&&!recombine)continue;const dist=Math.hypot(p.x-q.x,p.y-q.y);if(dist<=44&&dist<bestDist){bestDist=dist;best={type:'particle',id:q.id,mode:recombine?'recombineH':'pair'}}
 }
 for(const piece of state.pieces.values()){
  if(!piece.free||piece.moleculeId)continue;
  const bind=p.kind==='e'&&pieceCanBindElectron(piece),mixed=primordialMixedReaction(piece.sym,p.kind);if(!bind&&!mixed)continue;
  const radius=piece.matterState==='atom'?58:49,dist=Math.hypot(p.x-piece.x,p.y-piece.y);if(dist<=radius&&dist<bestDist){bestDist=dist;best={type:'piece',id:piece.id,mode:bind?'bindElectron':'mixed'}}
 }
 return best
}
function executePrimordialParticleDrop(p,target){
 if(!p||!target)return false;
 if(target.type==='particle'){const q=state.primordialParticles.get(target.id);if(!q||q.reacting)return false;if(target.mode==='recombineH'){recombineHydrogenParticles(p,q);return true}const r=primordialParticlePairReaction([p.kind,q.kind]);if(r){reactPrimordialParticlePair(r,p,q);return true}}
 if(target.type==='piece'){const piece=state.pieces.get(target.id);if(!piece||!piece.free)return false;if(target.mode==='bindElectron'&&p.kind==='e'&&pieceCanBindElectron(piece)){bindElectronToPiece(piece,p);return true}const r=primordialMixedReaction(piece.sym,p.kind);if(r){reactPrimordialMixed(r,piece,p);return true}}
 return false
}
...
function finishParticleDrag(id,ev,cancel=false){
 const d=state.particleDrag;if(!d||d.id!==id||d.pointerId!==ev.pointerId)return false;if(d.holdTimer)clearTimeout(d.holdTimer);const p=state.primordialParticles.get(id),wasActive=d.active,target=!cancel&&p?(d.target||primordialParticleDropTarget(p)):null;state.particleDrag=null;try{d.el.releasePointerCapture(ev.pointerId)}catch(e){}
 if(!p)return wasActive;if(wasActive){ev.preventDefault();ev.stopPropagation();p.dragging=false;p.suppressTapUntil=performance.now()+520;p.throwing=false;p.throwVx=0;p.throwVy=0;if(target&&executePrimordialParticleDrop(p,target)){renderPrimordialParticles();renderPieces();return true}const v=cancel?{x:0,y:0}:{x:d.vx,y:d.vy};if(Math.hypot(v.x,v.y)>.06)animateParticleThrow(id,v.x,v.y);else{renderPrimordialParticles();renderPieces();startPrimordialDrift()}return true}return false
}
...
function incomingParticlePoint(){const size=starSize(),rect=dom.star?.getBoundingClientRect?.(),edge=Math.floor(Math.random()*4);if(rect&&rect.width>0&&rect.height>0){const scale=size/rect.width,margin=Math.max(30,Math.min(window.innerWidth,window.innerHeight)*.06),clientX=edge===0?-margin:edge===1?window.innerWidth+margin:Math.random()*window.innerWidth,clientY=edge===2?-margin:edge===3?window.innerHeight+margin:Math.random()*window.innerHeight;return{x:(clientX-rect.left)*scale,y:(clientY-rect.top)*scale}}const margin=Math.max(34,size*.09),along=margin+Math.random()*Math.max(1,size-margin*2);if(edge===0)return{x:-margin,y:along};if(edge===1)return{x:size+margin,y:along};if(edge===2)return{x:along,y:-margin};return{x:along,y:size+margin}}
function spawnIncomingParticle(kind){const start=incomingParticlePoint(),p=createPrimordialParticle(kind,start.x,start.y),shell=!isPrimordial()&&['p','n','e'].includes(kind),dest=shell?stellarShellTarget(p,kind):freePoint(30);p.reacting=true;p.incoming=true;renderPrimordialParticles();requestAnimationFrame(()=>requestAnimationFrame(()=>{const q=state.primordialParticles.get(p.id);if(!q)return;q.x=dest.x;q.y=dest.y;renderPrimordialParticles()}));setTimeout(()=>{const q=state.primordialParticles.get(p.id);if(q){q.reacting=false;q.incoming=false;renderPrimordialParticles();startPrimordialDrift()}},920);return p}
function ensurePrimordialParticleMix({p=0,e=0,n=0}={},incoming=false){for(const [kind,min] of Object.entries({p,e,n}))while(countFloatingParticle(kind)<min)(incoming?spawnIncomingParticle(kind):createPrimordialParticle(kind))}
...
function ensureBaseMatterReserve(s=phase()){
 ...
 ensurePrimordialParticleMix(reserve,true);
...
function primordialGoalCount(s=phase()){if(s?.mode==='atomicRecombination')return state.atomicNeutralCreated?.[s.new]||0;return state.created[s.new]||0}
...
function prepareCumulativeStellarAtomicMatter(s=phase()){
 if(stellarAtomicMode(s)||!stellarAtomicChemistryAllowed(s)||!stellarIonizationKnowledge(s))return false;
 const outer=phaseRadius(s);let changed=false;
 state.pieces.forEach(p=>{
  if(!p||p.free||p.cell===null||p.cell===undefined||coords[p.cell]?.ring!==outer||p.matterState==='atom'||p.neutronBetaPending||pieceIsUnstable(p)||!E[p.sym]?.n)return;
  p.matterState='atom';p.boundElectrons=Number(E[p.sym].n||0);changed=true;
 });
 return changed
}
...
const STELLAR_CONTINUITY_POPULATION=15;
...
function clearBoard(){stopPrimordialMoleculeDrift();state.primordialMolecules?.clear();state.board=Array(coords.length).fill(null);state.pieces.clear();state.selected=[];state.freeSelected=[];state.created={};dom.lines.innerHTML='';dom.pieces.innerHTML=''}
...
function fillStage(){
  ...
  const s=phase();if(stellarAtomicMode(s)){fillStellarAtomicStage(s);return}
  ...
  renderPieces();renderPrimordialParticles();requestAnimationFrame(()=>{state.pieces.forEach(p=>{if(!p.free&&p.cell!==null&&p.cell!==undefined){const q=pos(coords[p.cell]);p.x=q.x;p.y=q.y}});renderPieces()});return;
   ...
   if(s.mode==='convection'){
    ...
    const edgeRing=phaseRadius(s),edgeCells=[];
    for(const [dq,dr] of dirs){const cell=coordIndex.get(`${dq*edgeRing},${dr*edgeRing}`);if(cell!==undefined&&!used.has(cell))edgeCells.push(cell)}
    if(s.coronalJetTutorial){
      const jetCell=edgeCells[0],neutralCell=edgeCells[1];
      if(jetCell!==undefined){createPiece('He',jetCell,false,{matterState:'atom',boundElectrons:1});used.add(jetCell)}
      if(neutralCell!==undefined){createPiece('He',neutralCell,false,{matterState:'atom',boundElectrons:2});used.add(neutralCell)}
      for(const cell of edgeCells.slice(2)){createPiece('H',cell,false,{matterState:'atom',boundElectrons:1});used.add(cell)}
    }else for(const cell of edgeCells){createPiece('H',cell,false);used.add(cell)}
    ...
    if(s.coronalJetTutorial){ensurePrimordialParticleMix({p:3,e:3,n:1});startPrimordialDrift();renderPrimordialParticles()}
...
function ensureProtonCaptureFuel(min=2){
 if(!protonCaptureAvailable())return;while(countFloatingParticle('p')<min)spawnIncomingParticle('p');renderPrimordialParticles();
}
...
async function attemptProtonCapture(cell,protonId){
 ...
 setTimeout(()=>{const q=state.pieces.get(target.id);if(q){q.newborn=false;renderPieces()}},360);await afterNuclearAction({advanceRound:true,protectedPieceIds:[target.id],reactionProductIds:[target.id]});state.protonCaptureAttempts[key]=0;ensureProtonCaptureFuel(s.mode==='protonCapture'?3:(rpStep(s)?.fuel==='p'?4:1));state.selected=[];state.locked=false;render();checkComplete();const chainCtx=state.chainAutoContext,chainRoot=chainCtx?.rootId||startChainEvent('proton',target.x,target.y),chainDepth=chainCtx?.depth||1;if(!pieceIsUnstable(target))scheduleAutoProtonCascade(target.id,chainRoot,chainDepth);
}
...
function learnedFusionRecipes(){
  ...
  if(current?.mode==='fusion')phaseFusionRecipes(current).forEach(r=>map.set(recipeKey(r),r));
    const addObjectiveDependency=(r,seen=new Set())=>{
...
function movementMechanicUnlocked(s=phase()){
...
async function moveSelectedAtom(targetCell){
 ...
 state.locked=true;state.board[source]=null;state.board[targetCell]=id;p.cell=targetCell;const q=pos(coords[targetCell]);p.x=q.x;p.y=q.y;state.selected=[];state.freeDrag=null;state.contextRecipeKey=null;tone(275,.055,'sine',.022);vibrate(5);dom.star.classList.add('pulse');render();
 await wait(300);
...
function coronalJetMechanicUnlocked(s=phase()){
 return convectionMechanicUnlocked(s)&&campaignKnowledgeReached('coronal_jets')
}
function coronalJetIonAtPathEdge(path,s=phase()){
 if(!coronalJetMechanicUnlocked(s)||!Array.isArray(path)||!path.length)return null;
 const outer=phaseRadius(s),cell=path.find(c=>Number.isInteger(c)&&coords[c]?.ring===outer);
 if(cell===undefined)return null;const id=state.board[cell],p=id?state.pieces.get(id):null;
 return p&&!p.free&&p.matterState==='atom'&&pieceCharge(p)>0?p:null
}
function coronalJetChargedCompanions(){
 const out=[];
 for(const kind of ['p','e']){const available=[...state.primordialParticles.values()].filter(p=>p.kind===kind&&!p.reacting);if(available.length>1)out.push(available[0])}
 const positron=[...state.primordialParticles.values()].find(p=>p.kind==='pos'&&!p.reacting);if(positron)out.push(positron);
 return out
}
async function maybeEjectCoronalJet(path,s=phase()){
 if(state.coronalJetRunning)return false;const ion=coronalJetIonAtPathEdge(path,s);if(!ion)return false;
 state.coronalJetRunning=true;await teachProductOnce('coronalJet',ion.x,ion.y);
 const cell=ion.cell,origin={x:ion.x,y:ion.y},source=dom.pieces.querySelector(`[data-id="${ion.id}"]`),clone=source?.cloneNode(true)||null,size=starSize(),c=size/2,a=Math.atan2(origin.y-c,origin.x-c),dist=size*1.08;
 if(clone){clone.removeAttribute('data-id');clone.style.pointerEvents='none';clone.style.left=origin.x+'px';clone.style.top=origin.y+'px';clone.style.zIndex='18';dom.fx.appendChild(clone)}
 const arc=document.createElement('div');arc.className='gamma-emission';arc.textContent='⌁';arc.style.left=origin.x+'px';arc.style.top=origin.y+'px';arc.style.fontSize='30px';dom.fx.appendChild(arc);
 const companions=coronalJetChargedCompanions();companions.forEach((q,i)=>{q.reacting=true;q.x=c+Math.cos(a+(i-.5)*.10)*size*1.02;q.y=c+Math.sin(a+(i-.5)*.10)*size*1.02});
 state.board[cell]=null;state.pieces.delete(ion.id);renderPieces();renderPrimordialParticles();
 if(clone)clone.animate([{transform:'translate(-50%,-50%) scale(1)',opacity:1},{transform:`translate(calc(-50% + ${Math.cos(a)*dist}px),calc(-50% + ${Math.sin(a)*dist}px)) scale(.34)`,opacity:0}],{duration:820,easing:'cubic-bezier(.16,.76,.2,1)',fill:'forwards'});
 arc.animate([{transform:'translate(-50%,-50%) scale(.5)',opacity:.9},{transform:`translate(calc(-50% + ${Math.cos(a)*dist*.82}px),calc(-50% + ${Math.sin(a)*dist*.82}px)) scale(1.8)`,opacity:0}],{duration:760,easing:'ease-out',fill:'forwards'});
 tone(720,.16,'sawtooth',.032);vibrate([8,18,12]);await wait(840);
 companions.forEach(q=>state.primordialParticles.delete(q.id));clone?.remove();arc.remove();
 if(s.coronalJetTutorial)state.coronalJetCount=(state.coronalJetCount||0)+1;
 registerRewardDiscovery('phenomenon:coronalJet',{title:'JATOS CORONAIS',text:'Plasma ionizado da superfície foi acelerado para fora da estrela.',silent:true});
 registerRewardDiscovery('phenomenon:magneticReconnection',{title:'RECONEXÃO MAGNÉTICA',text:'A reorganização do campo magnético liberou energia e acelerou o plasma.',silent:true});
...
function renderPieces(){
 ...
 state.pieces.forEach((p,id)=>{
  ...
  if(!el){
   ...
   el.addEventListener('pointerdown',ev=>{if(['neutronize','neutron','guidedDecay','decayGarden'].includes(phase().mode)||(isPostAtomMode()&&phase().mode!=='blackhole')){ev.stopPropagation();beginCrushHold(id,el,ev)}armPrimordialFreeDrag(id,el,ev);armStellarBoardDrag(id,el,ev)});
   el.addEventListener('pointermove',ev=>{movePrimordialFreeDrag(id,ev);moveStellarBoardDrag(id,ev)});
   el.addEventListener('pointerup',ev=>{finishPrimordialFreeDrag(id,ev,false);finishStellarBoardDrag(id,ev,false);cancelCrushHold(id)});el.addEventListener('pointercancel',ev=>{finishPrimordialFreeDrag(id,ev,true);finishStellarBoardDrag(id,ev,true);cancelCrushHold(id)});el.addEventListener('pointerleave',ev=>{if(ev.pointerType==='mouse')cancelCrushHold(id)});
  ...
  el.style.background=elementStyle(p.sym);const shownSym=pieceDisplaySymbol(p);el.style.setProperty('--symScale',pieceSymbolScale(p,shownSym));
...
function ensureCumulativeParticleFuel(s=phase()){
 ...
 if(s?.coronalJetTutorial){ensurePrimordialParticleMix({p:1,n:1,e:1},true);startPrimordialDrift();return}
 if(!s||isPrimordial(s)||!cumulativeParticleInteractionAllowed(s))return;
 ensurePrimordialParticleMix({p:2,n:2,e:atomicRecombinationLearned('H')?2:0},true);startPrimordialDrift();
}
function renderPrimordialParticles(){
 if(!dom.primordial)return;const s=phase(),primordialActive=isPrimordial(s)&&s.mode!=='opening',canUseCumulative=cumulativeParticleInteractionAllowed(s),canUseStellarProton=!!stellarProtonRecipe(s)||protonCaptureAvailable(s),canUseStellarAtomic=stellarAtomicChemistryAllowed(s),dragParticle=state.particleDrag?.active?state.primordialParticles.get(state.particleDrag.id):null,selectedParticle=dragParticle||(state.primordialSelected!==null?state.primordialParticles.get(state.primordialSelected):null),selectedPiece=state.freeSelected.length?state.pieces.get(state.freeSelected[0]):null,selectedBoardPiece=state.selected.length?state.pieces.get(state.board[state.selected[0]]):null;dom.primordial.classList.toggle('active',(primordialActive||canUseStellarProton||canUseCumulative||canUseStellarAtomic)&&!state.locked);const existing=new Map([...dom.primordial.querySelectorAll('.primordial-particle')].map(el=>[+el.dataset.id,el]));
 ...
 state.primordialParticles.forEach((p,id)=>{let el=existing.get(id);if(!el){el=document.createElement('button');el.type='button';el.dataset.id=id;el.addEventListener('contextmenu',ev=>ev.preventDefault());el.addEventListener('pointerdown',ev=>armParticleDrag(id,el,ev));el.addEventListener('pointermove',ev=>moveParticleDrag(id,ev));el.addEventListener('pointerup',ev=>finishParticleDrag(id,ev,false));el.addEventListener('pointercancel',ev=>finishParticleDrag(id,ev,true));el.addEventListener('click',ev=>{ev.preventDefault();ev.stopPropagation();const q=state.primordialParticles.get(id);if(q&&performance.now()<(q.suppressTapUntil||0))return;tapPrimordialParticle(id)});dom.primordial.appendChild(el)}const cls=p.kind==='p'?'proton':p.kind==='pos'?'positron':p.kind==='n'?'neutronfree':'electron',label=p.kind==='p'?'+':p.kind==='pos'?'e⁺':p.kind==='n'?'n':'−';let candidate=false,interactive=false;
   ...
   }else if((canUseStellarProton||canUseCumulative||canUseStellarAtomic)&&!p.reacting){
     if(selectedBoardPiece){candidate=!!primordialMixedReaction(selectedBoardPiece.sym,p.kind)||(p.kind==='p'&&((stellarProtonRecipe(s)&&selectedBoardPiece.sym==='H')||(protonCaptureAvailable(s)&&protonCaptureRoute(selectedBoardPiece,s))))||(p.kind==='e'&&(stellarIonizationEligible(selectedBoardPiece,s)||stellarRecombinationEligible(selectedBoardPiece,s)));interactive=candidate}
     else if(selectedParticle&&selectedParticle.id!==id){candidate=!!primordialParticlePairReaction([selectedParticle.kind,p.kind])||(atomicRecombinationLearned('H')&&same([selectedParticle.kind,p.kind],['p','e']));interactive=candidate||state.primordialSelected===id}
     else{interactive=learnedPrimordialNuclearReactions().some(r=>r.particles.includes(p.kind))||(p.kind==='e'&&(['H','He','Li'].some(atomicRecombinationLearned)||canUseStellarAtomic))||(p.kind==='p'&&(canUseStellarProton||phase().mode==='stellarRecombination'))}
   }
   const reserve=!isPrimordial(s)&&!stellarAtomicMode(s)&&['p','n','e'].includes(p.kind)&&!p.reacting&&!foregroundFree.has(id),dropTarget=(state.particleDrag?.active&&state.particleDrag.target?.type==='particle'&&state.particleDrag.target.id===id)||(state.freeDrag?.active&&state.freeDrag.target?.type==='particle'&&state.freeDrag.target.id===id);el.className=`primordial-particle ${cls}${p.kind==='n'&&p.unstable?' unstable':''}${state.primordialSelected===id?' selected':''}${candidate?' candidate':''}${dropTarget?' drop-target':''}${p.reacting?' reacting':''}${p.dragging?' dragging':''}${p.throwing?' throwing':''}${p.coulombDeflect?' coulomb-deflect':''}${p.tunneling?' tunneling':''}${reserve?' particle-reserve':''}`;el.textContent=label;el.style.pointerEvents=reserve?'none':'auto';el.setAttribute('aria-hidden',reserve?'true':'false');el.dataset.mechanical=interactive&&!reserve?'1':'0';el.style.left=p.x+'px';el.style.top=p.y+'px';existing.delete(id)});existing.forEach(el=>el.remove())
}
async function reactCumulativeProcessNeutronWithProton(proton,n){
 ...
 const spawn=createParticleReactionProduct('D',x,y,{massNumber:2}),out=spawn.piece;out.newborn=true;state.created.D=(state.created.D||0)+1;state.discovered.add('D');focusPieceInfo(out);burst(x,y);await emitGamma(x,y);await settleParticleReactionProduct(spawn);await afterNuclearAction({advanceRound:true,reactionProductIds:[out.id]});
...
function tapPrimordialParticle(id){
 ...
 const board=state.selected.length?state.pieces.get(state.board[state.selected[0]]):null;if(board){if(p.kind==='e'&&stellarIonizationEligible(board,s))return ionizeStellarAtom(board,p);if(p.kind==='e'&&stellarRecombinationEligible(board,s))return recombineStellarIon(board,p);const mixed=primordialMixedReaction(board.sym,p.kind);if(mixed)return reactCumulativeBoardMixed(mixed,board,p);if(p.kind==='p'){const protonRecipe=stellarProtonRecipe(s);if(board.sym==='H'&&protonRecipe){setTimeout(()=>fuseHydrogenWithProton(board.cell,p.id,protonRecipe),70);return}if(protonCaptureAvailable(s)&&protonCaptureRoute(board,s)){setTimeout(()=>attemptProtonCapture(board.cell,p.id),70);return}}}
 if(state.primordialSelected===id){state.primordialSelected=null;render();return}
 ...
 if(selectedPiece?.moleculeId){state.freeSelected=[];invalidPrimordial(id);render();return}
...
async function reactPrimordialParticlePair(r,a,b){
 if(state.locked||!a||!b||a.reacting||b.reacting)return;const s=phase(),x=(a.x+b.x)/2,y=(a.y+b.y)/2,goal=r.out===s.new&&primordialGoalCount(s)<Math.max(1,s.target||1);state.locked=true;state.primordialSelected=null;const motif=goal?await objectiveInteractionPrelude(`primordial:${s.id}:${r.out}`,[objectiveInteractionPrimordialToken(a),objectiveInteractionPrimordialToken(b)],r.out,{x,y}):null;const reactionPoint=r.id==='pn_d'&&motif?{x:motif.center,y:motif.center}:{x,y};
 a.reacting=true;b.reacting=true;if(!motif){a.x=x;a.y=y;b.x=x;b.y=y;renderPrimordialParticles();await wait(260)}state.primordialParticles.delete(a.id);state.primordialParticles.delete(b.id);renderPrimordialParticles();const spawn=createParticleReactionProduct(r.out,reactionPoint.x,reactionPoint.y,{massNumber:r.mass,longRadioactive:!!r.longRadioactive}),out=spawn.piece;out.newborn=true;focusPieceInfo(out);state.created[r.out]=(state.created[r.out]||0)+1;state.discovered.add(r.out);recordFlow(r.out===s.new?2:1);if(motif)await objectiveInteractionRevealPiece(motif,out,reactionPoint);else render();burst(reactionPoint.x,reactionPoint.y);await handleReactionEmissions(r,out);await settleParticleReactionProduct(spawn);await afterNuclearAction({advanceRound:true,reactionProductIds:[out.id]});setTimeout(()=>{const q=state.pieces.get(out.id);if(q){q.newborn=false;renderPieces()}},300);state.locked=false;render();checkComplete()
}
...
async function recombineHydrogenParticles(a,b){
 if(state.locked||a.reacting||b.reacting)return;const p=a.kind==='p'?a:b,e=a.kind==='e'?a:b;if(!p||!e)return;const s=phase(),x=(p.x+e.x)/2,y=(p.y+e.y)/2,moleculePhase=s.mode==='primordialMolecule';state.locked=true;state.primordialSelected=null;const motif=moleculePhase?null:await objectiveInteractionPrelude(`atomic:${s.id}:H`,[objectiveInteractionPrimordialToken(p),objectiveInteractionPrimordialToken(e)],'H',{x,y});p.reacting=true;e.reacting=true;if(moleculePhase){p.x=x;p.y=y;e.x=x;e.y=y;renderPrimordialParticles();await wait(145)}state.primordialParticles.delete(p.id);state.primordialParticles.delete(e.id);renderPrimordialParticles();const spawn=createParticleReactionProduct('H',x,y,{matterState:'atom',boundElectrons:1,massNumber:1}),h=spawn.piece;h.newborn=true;focusPieceInfo(h);state.created.H=(state.created.H||0)+1;if(s.mode==='atomicRecombination')state.atomicNeutralCreated.H=(state.atomicNeutralCreated.H||0)+1;state.discovered.add('H');recordFlow(1);if(s.mode==='stellarRecombination')creditStellarRecombination('H',s);if(motif)await objectiveInteractionRevealPiece(motif,h,{x,y});else render();burst(x,y);await emitGamma(x,y);await settleParticleReactionProduct(spawn);await afterNuclearAction({advanceRound:true,reactionProductIds:[h.id]});setTimeout(()=>{const q=state.pieces.get(h.id);if(q){q.newborn=false;renderPieces()}},300);state.locked=false;render();checkComplete()
}
async function bindElectronToPiece(piece,electron){
 if(state.locked||!pieceCanBindElectron(piece)||!electron||electron.kind!=='e')return;const s=phase(),x=piece.x,y=piece.y,nextBound=Math.min(E[piece.sym].n,Number(piece.boundElectrons||0)+1),finalNeutral=nextBound>=E[piece.sym].n,objectiveNeutral=s.mode==='atomicRecombination'&&piece.sym===s.new&&finalNeutral,moleculePhase=s.mode==='primordialMolecule';state.locked=true;state.freeSelected=[];state.primordialSelected=null;const key=`atomic:${s.id}:${piece.sym}:e`;const motif=!moleculePhase&&objectiveNeutral?await objectiveInteractionPrelude(key,[objectiveInteractionPieceToken(piece),objectiveInteractionPrimordialToken(electron)],piece.sym,{x,y}):null;if(!moleculePhase&&!objectiveNeutral)await objectiveInteractionImpact(key,[objectiveInteractionPieceToken(piece),objectiveInteractionPrimordialToken(electron)],piece.sym,{x,y},'e⁻','CAPTURA');electron.reacting=true;if(moleculePhase){electron.x=x;electron.y=y;renderPrimordialParticles();await wait(120)}state.primordialParticles.delete(electron.id);renderPrimordialParticles();piece.boundElectrons=nextBound;piece.matterState='atom';piece.newborn=true;focusPieceInfo(piece);recordFlow(1);if(pieceCharge(piece)===0){state.created[piece.sym]=(state.created[piece.sym]||0)+1;if(s.mode==='atomicRecombination')state.atomicNeutralCreated[piece.sym]=(state.atomicNeutralCreated[piece.sym]||0)+1;state.discovered.add(piece.sym)}if(motif)await objectiveInteractionRevealPiece(motif,piece,{x,y});else renderPieces();burst(piece.x,piece.y);await emitGamma(piece.x,piece.y);await afterNuclearAction({advanceRound:true,reactionProductIds:[piece.id]});setTimeout(()=>{const q=state.pieces.get(piece.id);if(q){q.newborn=false;renderPieces()}},300);state.locked=false;render();checkComplete()
}

async function ionizeStellarAtom(piece,electron){
 const s=phase();if(state.locked||state.phaseDone||!electron||electron.kind!=='e'||electron.reacting||!stellarIonizationEligible(piece,s))return false;const sym=piece.sym,x=piece.x,y=piece.y,cell=piece.cell;if(s.id==='stellar_ionization'&&!state.productLessons.has('stellarIonization')){await teachProductOnce('stellarIonization',x,y);if(phase()!==s||state.phaseDone)return false}state.locked=true;state.selected=[];state.primordialSelected=null;electron.reacting=true;await objectiveInteractionImpact(`stellar-ionization:${s.id}:${sym}`,[objectiveInteractionPieceToken(piece),objectiveInteractionPrimordialToken(electron)],sym,{x,y},sym==='H'?'p⁺ + e⁻':`${sym}⁺ + e⁻`,'IONIZAÇÃO');
...
function startPrimordialMoleculeDrift(){
 stopPrimordialMoleculeDrift();if(!rotationMotionEnabled()||!state.primordialMolecules?.size)return;state.primordialMoleculeTimer=setInterval(()=>{const s=phase();if(!rotationMotionEnabled()||s.mode!=='primordialMolecule'||state.phaseDone||state.locked)return;const size=starSize(),pad=Math.max(62,size*.13),selected=selectedPrimordialMolecule(),dragged=state.freeDrag?.active?state.freeDrag.moleculeId:null;for(const m of state.primordialMolecules.values()){if(selected?.id===m.id||dragged===m.id)continue;const step=12+Math.random()*13,a=Math.random()*Math.PI*2;m.x=Math.max(pad,Math.min(size-pad,m.x+Math.cos(a)*step));m.y=Math.max(pad,Math.min(size-pad,m.y+Math.sin(a)*step));m.angle+=(Math.random()-.5)*.28;positionPrimordialMolecule(m)}renderPieces();syncPrimordialMoleculeVisuals()},720);
}
...
function primordialFreeDragTarget(d){
 if(!d?.active||!isPrimordial()||phase().mode==='opening')return null;const s=phase(),source=d.moleculeId?null:state.pieces.get(d.sourceId),molecule=d.moleculeId?state.primordialMolecules.get(d.moleculeId):null,origin=molecule||source;if(!origin)return null;let best=null,bestDist=Infinity;
 if(molecule){
  if(s.id!=='first_nebulae'||molecule.locked||molecule.type!=='HeH+')return null;
  for(const p of state.pieces.values()){if(!primordialNeutralAtom(p,'H'))continue;const dist=Math.hypot(origin.x-p.x,origin.y-p.y);if(dist<=68&&dist<bestDist){bestDist=dist;best={type:'piece',id:p.id,pieceIds:[p.id],mode:'heh+h'}}}
  return best
 }
 for(const p of state.pieces.values()){
  if(!p.free||p.id===source.id)continue;
  const targetMolecule=primordialMoleculeForPiece(p);
  if(targetMolecule){if(s.id==='first_nebulae'&&!targetMolecule.locked&&targetMolecule.type==='HeH+'&&primordialNeutralAtom(source,'H')){const dist=Math.hypot(source.x-targetMolecule.x,source.y-targetMolecule.y);if(dist<=68&&dist<bestDist){bestDist=dist;best={type:'molecule',id:targetMolecule.id,pieceIds:[...targetMolecule.members],mode:'h+heh'}}}continue}
  const bond=canCreatePrimordialHeH(source,p,s),reaction=primordialPieceReaction([source.sym,p.sym]);if(!bond&&!reaction)continue;const dist=Math.hypot(source.x-p.x,source.y-p.y);if(dist<=60&&dist<bestDist){bestDist=dist;best={type:'piece',id:p.id,pieceIds:[p.id],mode:bond?'heh':'nuclear'}}
 }
 for(const particle of state.primordialParticles.values()){
  if(particle.reacting||particle.dragging)continue;const bind=particle.kind==='e'&&pieceCanBindElectron(source),reaction=primordialMixedReaction(source.sym,particle.kind);if(!bind&&!reaction)continue;const dist=Math.hypot(source.x-particle.x,source.y-particle.y);if(dist<=58&&dist<bestDist){bestDist=dist;best={type:'particle',id:particle.id,pieceIds:[],mode:bind?'bindElectron':'mixed'}}
 }
 return best
}
function executePrimordialFreeDrop(d,target){
 if(!d||!target)return false;const source=d.moleculeId?null:state.pieces.get(d.sourceId),molecule=d.moleculeId?state.primordialMolecules.get(d.moleculeId):null;
 if(molecule&&target.mode==='heh+h'){const h=state.pieces.get(target.id);if(h){reactPrimordialHeHWithHydrogen(molecule,h);return true}}
 if(source&&target.type==='molecule'&&target.mode==='h+heh'){const m=state.primordialMolecules.get(target.id);if(m){reactPrimordialHeHWithHydrogen(m,source);return true}}
 if(source&&target.type==='particle'){const particle=state.primordialParticles.get(target.id);if(!particle||particle.reacting)return false;if(target.mode==='bindElectron'&&particle.kind==='e'&&pieceCanBindElectron(source)){bindElectronToPiece(source,particle);return true}const r=primordialMixedReaction(source.sym,particle.kind);if(r){reactPrimordialMixed(r,source,particle);return true}}
 if(source&&target.type==='piece'){const p=state.pieces.get(target.id);if(!p)return false;if(target.mode==='heh'&&canCreatePrimordialHeH(source,p)){formPrimordialHeH(source,p);return true}const r=primordialPieceReaction([source.sym,p.sym]);if(r){fuseFree(r,[source.id,p.id]);return true}}
 return false
}
function cancelPrimordialFreeDrag(){
 const d=state.freeDrag;if(!d)return;state.freeDrag=null;if(d.moleculeId){const m=state.primordialMolecules.get(d.moleculeId);if(m)m.dragging=false}renderPieces();syncPrimordialMoleculeVisuals()
}
...
function movePrimordialFreeDrag(id,ev){
 ...
 d.target=primordialFreeDragTarget(d);renderPieces();syncPrimordialMoleculeVisuals()
}
...
function tapFreeAtom(id){
 ...
 const particle=state.primordialSelected!==null?state.primordialParticles.get(state.primordialSelected):null;
...
function neutronTrajectoryCandidate(n,s=phase()){
 ...
 for(const p of state.pieces.values()){if(p.free||p.cell===null||p.cell===undefined||!(neutronEligible(p,s)||universalNeutronCaptureEligible(p))||!neutronAutoTargetPreviouslyLearned(p,s))continue;const dx=p.x-n.x,dy=p.y-n.y,forward=dx*ux+dy*uy;if(forward<0||forward>starSize()*.92)continue;const cross=Math.abs(dx*uy-dy*ux);if(cross>tol)continue;options.push({p,score:cross+forward*.025})}
...
function neutronPhaseNextRecipeLine(s=phase()){
 ...
 const pendingAll=[...state.pieces.values()].filter(p=>p.neutronBetaPending);
...
function conciseRecipeLine(s=phase()){
 ...
 if(s.mode==='fusion'){
   ...
   const r=contextualObjectiveRecipe(s);if(r&&hasRecipeIngredients(r,guidanceBoardSymbolCounts()))return topFusionLabel(r);const missing=target?orderedSpatialNeeds(target,s).find(n=>n.missing>0):null;if(missing)return `Reconstrua ${E[missing.sym]?.name||missing.sym}`;
 }
 return s.meta||'';
}
function stageProgressLabel(s=phase()){
 if(state.readyToAdvance){if(s.mode==='stellarFormation')return'ESTRELA PRONTA';if(s.id==='brown')return'RESERVATÓRIO ESGOTADO';return'CONCLUÍDA'}
 if(s.id==='solar_wind')return'VENTO SOLAR';if(s.mode==='stellarIonization')return'IONIZAÇÃO';if(s.mode==='stellarRecombination')return'RECOMBINAÇÃO';
 if(s.mode==='stellarFormation')return'AGLOMERAÇÃO';if(s.mode==='primordialMolecule')return'QUÍMICA PRIMORDIAL';if(s.id==='brown')return'QUEIMA DE DEUTÉRIO';
 if(s.coronalJetTutorial)return'JATOS CORONAIS';if(s.mode==='convection')return'CONVECÇÃO';return''
}
function stageProgressMetric(s=phase(),{formationSpec=null,formationLargest=0}={}){
 const count=(current,total,detail='')=>({current:Math.max(0,Number(current)||0),total:Math.max(0,Number(total)||0),detail});
 if(s.mode==='opening'||s.mode==='showcase'||s.mode==='collapseFinal')return{current:null,total:null,detail:''};
 if(s.mode==='movementTutorial')return count(objectiveSatisfied(s)?1:0,s.target||1);
 if(s.mode==='stellarFormation')return count(formationLargest,formationSpec?.total||stellarFormationSpec(s).total);
 if(s.mode==='stellarIonization')return count(state.stellarIonizations,s.target);
 if(s.mode==='stellarRecombination')return count(state.stellarRecombinations,s.target);
 if(s.coronalJetTutorial)return count(state.coronalJetCount||0,s.target);
 if(s.mode==='convection')return count(state.convectionMoves||0,s.target);
 if(s.mode==='reactionExplore')return count(state.atlasProgress||0,s.target);
 if(s.mode==='primordialMolecule'||s.mode==='primordialNuclear'||s.mode==='atomicRecombination')return count(primordialGoalCount(s),s.target);
 if(s.mode==='whiteCompact'){const w=whiteCounts(s),c=Math.min(w.c,w.targetC),o=Math.min(w.o,w.targetO);return count(c+o,w.targetC+w.targetO,`C ${c} de ${w.targetC} · O ${o} de ${w.targetO}`)}
 if(s.mode==='neutronize')return count(state.crushed,s.target);
 if(s.mode==='blackhole'||['remnant','pulsar','accretion'].includes(s.mode))return count(state.absorbed,s.target);
 if(s.mode==='protonCapture')return count(state.protonCaptures,s.target);
 if(s.mode==='rpProcess'){const made=state.created[s.new]||0,step=rpStep(s),parts=[`${Math.min(made,s.target)} de ${s.target}`];if(step?.pattern==='waiting')parts.push(`waiting point ${Math.min(1,state.rpWaitDecays)} de 1`);if(step?.pattern==='cycle')parts.push(`ciclo ${Math.min(1,state.rpCyclesObserved)} de 1`);return count(made,s.target,parts.join(' · '))}
 if(s.mode==='decayGarden')return count(decayDiscoveryCount(s),s.target);
 if(s.mode==='neutron'){const g=neutronGameplay(s),made=state.created[s.new]||0,parts=[`${Math.min(made,s.target)} de ${s.target}`];if(s.id==='tc'||s.id==='pm')parts.push(`radioatividade ${state.radioactiveProofDone?1:0} de 1`);if(g.requiresSource)parts.push(`fonte ${Math.min(1,state.neutronSourceActivations)} de 1`);if(g.requiresBranch)parts.push(`ramificação ${Math.min(1,state.neutronBranchesObserved)} de 1`);if(g.requiresFreezeout)parts.push(`freeze-out ${Math.min(1,state.neutronFreezeouts)} de 1`);return count(made,s.target,parts.join(' · '))}
 if(s.id==='brown')return count(state.created.He3||0,brownBurnLimit());
 if(Number.isFinite(Number(s.target))&&Number(s.target)>0)return count(state.created[s.new]||0,s.target);
 return{current:null,total:null,detail:''}
}
function renderStageProgress({percent=0,current=null,total=null,label='',detail='',ready=false,visible=true}={}){
...
function updateObjective(){
 const s=phase();
 if(state.readyToAdvance){setFormula('Fase concluída com sucesso');return}
 if(s.mode==='movementTutorial'){$('goalText').textContent='Leve o Hélio até o núcleo estelar.';setFormula('Clique no átomo e avance um espaço por vez');return}
 ...
 if(s.mode==='primordialNuclear'){$('goalText').textContent=`Forme ${s.target} ${s.target===1?'núcleo':'núcleos'} de ${E[s.new].name}`;setFormula(conciseRecipeLine(s));return}
 if(s.mode==='atomicRecombination'){$('goalText').textContent=`Forme ${s.target} ${s.target===1?'átomo':'átomos'} de ${E[s.new].name}`;setFormula(conciseRecipeLine(s));return}
 if(s.id==='coulomb_intro'){$('goalText').textContent=`Crie ${s.target} núcleos estáveis de Hélio por Fusão`;setFormula('Hélio-3 + Hélio-3 → Hélio-4 + 2 prótons');return}
 if(s.coronalJetTutorial){$('goalText').textContent='Ejete matéria pelo campo magnético';const outer=phaseRadius(s),ready=[...state.pieces.values()].some(p=>!p.free&&p.cell!==null&&p.cell!==undefined&&coords[p.cell]?.ring===outer&&p.matterState==='atom'&&pieceCharge(p)>0);if(!ready)setFormula('Átomo + e⁻ → Íon⁺ + 2e⁻');else if(state.convectionCharge)setFormula('2º Ative a Convecção e selecione um íon na superfície.');else setFormula('1º Provoque uma reação no núcleo estelar');return}
 if(s.mode==='convection'){$('goalText').textContent=`Realize ${s.target} correntes convectivas`;setFormula(conciseRecipeLine(s));return}
 if(s.id==='stellar_li'){$('goalText').textContent=`Produza ${s.target} núcleos de Lítio-7 pelo mecanismo Cameron–Fowler`;setFormula(conciseRecipeLine(s));return}
 if(s.mode==='protonCapture'){$('goalText').textContent=`Realize ${s.target} capturas de prótons`;setFormula(conciseRecipeLine(s));return}
 if(s.mode==='rpProcess'){const step=rpStep(s),suffix=step?.pattern==='waiting'?' · observe o waiting point':step?.pattern==='cycle'?' · observe o ciclo terminal':'';$('goalText').textContent=`Forme ${s.target} núcleos de ${E[s.new].name}${suffix}`;setFormula(conciseRecipeLine(s));return}
 if(s.mode==='spallation'){$('goalText').textContent=`Produza ${s.target} núcleos de ${E[s.new].name}`;setFormula(conciseRecipeLine(s));return}
 if(s.mode==='neutrino'){$('goalText').textContent=`Produza ${s.target} núcleos de Flúor`;setFormula(conciseRecipeLine(s));return}
 if(s.mode==='gamma'){$('goalText').textContent=`Realize ${s.target} fotodesintegrações`;setFormula(conciseRecipeLine(s));return}
 if(s.mode==='guidedDecay'){$('goalText').textContent=`Crie ${s.target} ${s.target===1?'átomo':'átomos'} de ${E[s.new].name} por decaimento`;setFormula(conciseRecipeLine(s));return}
 if(s.mode==='decayGarden'){$('goalText').textContent=`Descubra ${s.target} descendentes`;setFormula(conciseRecipeLine(s));return}
 if(s.mode==='explosive'){$('goalText').textContent=`Produza ${s.target} núcleos de Zinco`;setFormula(conciseRecipeLine(s));return}
 if(s.mode==='whiteCompact'){const info=whiteCounts(s);$('goalText').textContent=`Crie ${info.targetC} Carbonos e ${info.targetO} Oxigênios`;setFormula(conciseRecipeLine(s));return}
 if(s.mode==='showcase'){$('goalText').textContent='Observe o remanescente';setFormula(conciseRecipeLine(s));return}
 if(s.mode==='neutronize'){$('goalText').textContent=`Comprima ${s.target} núcleos em nêutrons`;setFormula(conciseRecipeLine(s));return}
 if(s.mode==='remnant'){$('goalText').textContent=`Comprima ${s.target} núcleos`;setFormula(conciseRecipeLine(s));return}
 if(s.mode==='pulsar'){$('goalText').textContent=`Incorpore ${s.target} núcleos`;setFormula(conciseRecipeLine(s));return}
 if(s.mode==='accretion'){$('goalText').textContent='Alimente o remanescente';setFormula(conciseRecipeLine(s));return}
 if(s.mode==='collapseFinal'){$('goalText').textContent='Supere o limite de estabilidade';setFormula(conciseRecipeLine(s));return}
 if(s.mode==='blackhole'){$('goalText').textContent=`Atraia ${s.target} átomos ao Buraco Negro`;setFormula(conciseRecipeLine(s));return}
 const made=state.created[s.new]||0;
 if(s.uniqueMatterObjective){$('goalText').textContent=`Observe ${s.target} formações independentes de ${E[s.new].name}`;setFormula(conciseRecipeLine(s));return}
 if(s.id==='brown'){$('goalText').textContent=`Queime ${s.target} núcleos de Deutério`;setFormula(conciseRecipeLine(s));return}
 if(s.id==='he_red'){$('goalText').textContent=`Crie ${s.target} núcleos estáveis de Hélio por Fusão`;setFormula(conciseRecipeLine(s));return}
 if(s.id==='he_orange'){$('goalText').textContent=`Crie ${s.target} núcleos de Hélio-3 por Fusão`;setFormula(conciseRecipeLine(s));return}
 if(s.id==='he_yellow'){$('goalText').textContent=`Crie ${s.target} núcleos estáveis de Hélio por Fusão`;setFormula(conciseRecipeLine(s));return}
 if(s.mode==='neutron'){const g=neutronGameplay(s),extras=[];if(g.requiresSource)extras.push('ative uma fonte de nêutrons');if(g.requiresBranch)extras.push('observe uma ramificação');if(g.requiresFreezeout)extras.push('observe o freeze-out');$('goalText').textContent=`Crie ${s.target} ${E[s.new].name}${extras.length?' · '+extras.join(' · '):''}`;setFormula(conciseRecipeLine(s));return}
 $('goalText').textContent=s.mode==='fusion'?`Crie ${s.target} ${s.target===1?'átomo':'átomos'} de ${E[s.new].name} por Fusão`:`Crie ${s.target} ${s.target===1?'átomo':'átomos'} de ${E[s.new].name}`;setFormula(conciseRecipeLine(s));
}
function applyVisual(){const s=phase(),v=s.visual,root=document.documentElement,styles={nebula:['#fff3bc','#ffae5f','#4d2e82','rgba(255,126,72,.27)',.96],brownDwarf:['#ffe0a6','#9f532d','#351d31','rgba(169,86,47,.30)',.72],redDwarf:['#fff0ca','#ff7757','#6c2231','rgba(255,84,65,.38)',.84],orangeDwarf:['#fff6ce','#ffad56','#8c3e28','rgba(255,145,67,.42)',.91],yellowDwarf:['#fffbe0','#ffd66e','#a25228','rgba(255,199,81,.48)',1],whiteDwarf:['#ffffff','#dff2ff','#819bc2','rgba(210,237,255,.62)',.72],solar:['#fff9ce','#ffd06c','#9a4e28','rgba(255,192,83,.4)',1],redGiant:['#fff0b0','#ff7e48','#771d28','rgba(255,80,46,.48)',1.06],agb:['#ffe7a3','#f06d42','#6c2148','rgba(255,88,55,.42)',1.04],massive:['#eff8ff','#93c8ff','#3340a1','rgba(99,160,255,.48)',1.02],supergiant:['#fff','#b1d6ff','#4b4ac7','rgba(111,158,255,.58)',1.07],advanced:['#fffbd7','#ffb64f','#8d2843','rgba(255,132,67,.62)',1.08],ironCore:['#fff','#ff975a','#521722','rgba(255,72,47,.65)',1.08],kilonova:['#fff','#a6d8ff','#9e54d8','rgba(177,110,255,.6)',.96],interstellar:['#dfeaff','#6686c9','#18254d','rgba(85,129,215,.28)',.96],neutronStar:['#eaf9ff','#78baff','#1a2d80','rgba(104,178,255,.55)',.90],pulsar:['#fff','#89c9ff','#203ca3','rgba(128,199,255,.68)',.90],accretion:['#fff6cf','#ffb24e','#491d42','rgba(255,142,61,.58)',.92],xrayBurst:['#fffbe6','#8fd6ff','#44206d','rgba(132,206,255,.68)',.94],collapseFinal:['#fff','#ff8b62','#270b18','rgba(255,74,57,.66)',.88],blackHole:['#1b2543','#070914','#000','rgba(89,126,255,.20)',.90]};const a=styles[v]||styles.solar,prebang=s.mode==='opening'&&!state.bigBangColorized;document.body.classList.toggle('prebang',prebang);document.body.classList.toggle('bigbang-phase',s.mode==='opening');const spaceColors={bigBang:'#e10b17',primordialH:'#9f0814',primordialHe:'#4e0710',primordialLi:'#160307'};root.style.setProperty('--spaceBg',prebang?'#fff':(spaceColors[v]||'#000'));root.style.setProperty('--primordialGlow',prebang?'rgba(255,255,255,0)':v==='bigBang'?'rgba(255,210,130,.58)':v==='primordialH'?'rgba(255,80,40,.36)':v==='primordialHe'?'rgba(201,41,31,.28)':'rgba(112,24,29,.20)');dom.star.classList.toggle('primordial-mode',isPrimordial(s));dom.star.classList.toggle('primordial-molecule-mode',s.mode==='primordialMolecule');dom.star.classList.toggle('spallation-mode',['spallation','neutrino','gamma'].includes(s.mode));dom.star.classList.toggle('decay-mode',s.mode==='guidedDecay');dom.star.classList.toggle('electron-network',s.mode==='guidedDecay'||s.mode==='whiteCompact');dom.star.classList.toggle('white-electron-network',s.mode==='whiteCompact');dom.star.classList.toggle('white-structure',s.mode==='whiteCompact');dom.star.classList.toggle('cumulative-shells',s.mode!=='fusion'&&s.mode!=='whiteCompact'&&fusionSandboxAllowed(s)&&stratificationStrength(s)>0);dom.star.classList.toggle('large-board-uniform-atoms',phaseRadius(s)>=4);root.style.setProperty('--starA',a[0]);root.style.setProperty('--starB',a[1]);root.style.setProperty('--starC',a[2]);
  let glow=a[3];
  if(s.mode==='whiteCompact'){
...
function render(){drawLines();renderPieces();updateMoveTargets();renderPrimordialParticles();renderCosmicRays();updateHUD();renderNeutrons();renderConvectionControl();renderMenu();queueSolarWindCheck()}
...
function tapAtom(id){if(state.locked)return;const p=state.pieces.get(id);if(!p)return;focusPieceInfo(p);const s=phase();if(state.convectionArmed&&handleConvectionTap(p))return;if(handleStellarAtomicTap(p,s))return;if(p.free&&cumulativeParticleInteractionAllowed(s))return tapFreeAtom(id);if((p.sym==='Tc'||p.sym==='Pm')&&p.radioactiveReady)return tapRadioactiveProof(p);if(s.mode!=='reactionExplore'&&cumulativeFusionTapAvailable(p,s)&&handleFusionTap(p))return;if(s.mode==='reactionExplore'){if(tapAtlasReaction(p))return;if(fusionSandboxAllowed(s)&&handleFusionTap(p))return;if(selectAtomForMovement(p))return;invalid(p.cell);return}
 ...
 if(state.primordialSelected===null&&boardParticleTargetAvailable(p,s)){selectBoardParticleTarget(p);return}
 if(s.mode==='rpProcess'){
...
const PRODUCT_LESSONS={
 ...
 solarWind:{title:'VENTO SOLAR',text:'A coroa estelar é um plasma extremamente quente. Quando partículas desse plasma ganham energia suficiente, elas podem escapar continuamente para o espaço como vento solar.'},
 ...
 coronalJet:{title:'JATOS CORONAIS',text:'A convecção pode deformar estruturas magnéticas que alcançam a atmosfera estelar. Quando linhas de campo se reconectam, plasma ionizado na superfície pode ser acelerado para fora em um jato estreito.'}
};
...
function closeEventTooltip(){
 ...
 if((isPrimordial()&&phase().mode!=='opening')||protonCaptureAvailable(phase())||stellarAtomicChemistryAllowed(phase()))startPrimordialDrift();
...
async function relocateNewbornCoreProduct(reactionProductIds=[]){
 if(isPrimordial()||phaseRadius()<1)return[];
 const center=(byRing[0]||[])[0];if(center===undefined)return[];
 const id=state.board[center],piece=id?state.pieces.get(id):null,explicitProducts=new Set(reactionProductIds||[]);
 // A identidade causal da reação é o contrato principal. "newborn" permanece como
 // fallback visual para rotas antigas, mas timers de animação jamais decidem a geometria.
 if(!piece||piece.free||(!piece.newborn&&!explicitProducts.has(piece.id)))return[];
...
async function afterNuclearAction({advanceRound=false,forceBoardPulse=false,replenish=true,protectedPieceIds=[],reactionProductIds=[]}={}){
 // Produtos de reação formados no núcleo central saem para a primeira camada para
 // permanecerem visíveis e acessíveis, fora da área ocupada pelo controle central.
 const relocated=await relocateNewbornCoreProduct(reactionProductIds),protectedIds=[...new Set([...(protectedPieceIds||[]),...relocated])];
...
function ensureOpportunity(){
  ...
  if(s.id==='he_red'){
    const c=boardSymbolCounts();if((c.HeU||0)>=2)return true;if((c.H||0)===0){const cell=activeCells().find(i=>state.board[i]===null);if(cell!==undefined)createPiece('H',cell,true)}while(countFloatingParticle('p')<2)spawnIncomingParticle('p');render();return true
  }
  if(s.id==='coulomb_intro'){
...
function clearNeutronPending(piece){if(!piece)return;piece.neutronBetaPending=false;piece.neutronBetaReadyRound=null;piece.neutronBetaStartRound=null;piece.neutronBetaTotalRounds=null;piece.neutronBetaTransition=null;piece.neutronShellExposure=0;piece.neutronShellOpen=false}
...
function spawnNeutron(){
 const s=phase();if(s.mode!=='neutron'||state.phaseDone||state.popupOpen||state.readyToAdvance)return false;
 const size=starSize(),c=size/2,a=Math.random()*Math.PI*2,r=size*.60,id=state.nextN++,x=c+Math.cos(a)*r,y=c+Math.sin(a)*r,speed=1.35+Math.random()*.55;
 state.neutrons.set(id,{id,x,y,vx:-Math.cos(a)*speed+(Math.random()-.5)*.18,vy:-Math.sin(a)*speed+(Math.random()-.5)*.18,generated:false,incoming:true});renderNeutrons();return true
}
...
function moveNeutrons(){
 ...
 for(const n of state.neutrons.values()){
   ...
   if(n.incoming){n.x+=(n.vx||0);n.y+=(n.vy||0);const dx=n.x-c,dy=n.y-c,d=Math.hypot(dx,dy)||1;if(d<=limit){n.incoming=false;primeStellarShellParticle(n,'n')}continue}
...
async function captureNeutron(id){
 ...
 await afterNuclearAction({advanceRound:true,reactionProductIds:[p.id]});
...
// Reusable stellar-formation family. Visible H-H pairs provide every slot except
...
function stellarFormationSpec(s=phase()){
 ...
 const initialGroups=Math.min(pairCount,pairCount<=3?2:pairCount<=9?3:pairCount<=18?4:5),maxConcurrentGroups=Math.min(pairCount,pairCount<=3?2:pairCount<=9?4:pairCount<=18?5:6);
 return{layers,layersOutsideCore,radius,total,visibleTotal,pairCount,initialGroups,maxConcurrentGroups};
}
...
function stellarFormationAtomSize(spec=stellarFormationSpec()){
 const cap=spec.pairCount<=3?38:spec.pairCount<=9?36:spec.pairCount<=18?32:30,floor=spec.pairCount>18?24:26,fit=starSize()*.72/(2*spec.radius+3);
...
function stellarFormationVisualRadius(g,spec=stellarFormationSpec()){const pts=stellarFormationLayout(g.members.length,spec),extent=Math.max(0,...pts.map(p=>Math.hypot(p.x,p.y)));return extent+stellarFormationAtomSize(spec)*.5}
...
function removeStellarFormation(){const f=state.stellarFormation;if(f?.raf)cancelAnimationFrame(f.raf);if(f?.inflowTimer)clearTimeout(f.inflowTimer);f?.layer?.remove();state.stellarFormation=null;dom.star.parentElement?.classList.remove('formation-ready-shell');dom.star.classList.remove('stellar-formation-mode','formation-materializing','formation-birth-flash','formation-stabilized')}
function stellarFormationElementLabel(sym){return E[sym]?.symbol||sym}
function stellarFormationMakeField(f,g){const d=document.createElement('div');d.className='formation-g-field'+(g.incoming?' formation-incoming':'');d.dataset.group=String(g.id);f.fields.appendChild(d);g.fieldEl=d;return d}
function stellarFormationUpdateHud(){updateHUD()}
function stellarFormationGroupDraggable(g){return !!g&&!g.incoming&&g.members.length<=12}
function stellarFormationPointerPoint(ev){const r=dom.star.getBoundingClientRect(),scale=starSize()/Math.max(1,r.width);return{x:(ev.clientX-r.left)*scale,y:(ev.clientY-r.top)*scale}}
function stellarFormationDragTarget(f,g){if(!f||!g)return null;return[...f.groups.values()].filter(q=>q.id!==g.id&&!q.incoming&&stellarFormationOverlap(g,q)).sort((a,b)=>Math.hypot(g.x-a.x,g.y-a.y)-Math.hypot(g.x-b.x,g.y-b.y))[0]||null}
function armStellarFormationDrag(atomId,el,ev){
 const f=state.stellarFormation,a=f?.atoms.get(atomId),g=a?f?.groups.get(a.groupId):null;if(!f||!a||!g||!stellarFormationGroupDraggable(g)||state.locked||state.phaseDone||f.stabilizing||f.complete||ev.pointerType==='mouse'&&ev.button!==0)return;
 const pt=stellarFormationPointerPoint(ev);f.drag={atomId,groupId:g.id,pointerId:ev.pointerId,el,active:false,startX:pt.x,startY:pt.y,offsetX:g.x-pt.x,offsetY:g.y-pt.y,targetId:null};try{el.setPointerCapture(ev.pointerId)}catch(e){}
}
function moveStellarFormationDrag(atomId,ev){
 const f=state.stellarFormation,d=f?.drag;if(!f||!d||d.atomId!==atomId||d.pointerId!==ev.pointerId)return;const g=f.groups.get(d.groupId);if(!g)return;const pt=stellarFormationPointerPoint(ev);if(!d.active&&Math.hypot(pt.x-d.startX,pt.y-d.startY)<7)return;if(!d.active){d.active=true;f.selectedGroup=null;g.vx=0;g.vy=0;vibrate(5)}
 ev.preventDefault();ev.stopPropagation();const center=starSize()/2,r=Math.min(center-10,stellarFormationFieldRadius(g)),maxDist=Math.max(0,center-r-7),rawX=pt.x+d.offsetX,rawY=pt.y+d.offsetY,dx=rawX-center,dy=rawY-center,dist=Math.hypot(dx,dy);if(dist>maxDist&&dist>0){g.x=center+dx/dist*maxDist;g.y=center+dy/dist*maxDist}else{g.x=rawX;g.y=rawY}const target=stellarFormationDragTarget(f,g);d.targetId=target?.id||null;stellarFormationRefreshCompatibility(f)
}
function finishStellarFormationDrag(atomId,ev,cancel=false){
 const f=state.stellarFormation,d=f?.drag;if(!f||!d||d.atomId!==atomId||d.pointerId!==ev.pointerId)return false;const g=f.groups.get(d.groupId),wasActive=d.active,target=!cancel&&g&&d.targetId?f.groups.get(d.targetId):null;f.drag=null;try{d.el.releasePointerCapture(ev.pointerId)}catch(e){}if(!wasActive){stellarFormationRefreshCompatibility(f);return false}
 ev.preventDefault();ev.stopPropagation();const a=f.atoms.get(atomId);if(a)a.suppressTapUntil=performance.now()+520;if(!g)return true;if(target&&stellarFormationOverlap(g,target)){stellarFormationMerge(g,target);return true}g.vx=(Math.random()-.5)*.018;g.vy=(Math.random()-.5)*.018;stellarFormationRefreshCompatibility(f);return true
}
function stellarFormationRefreshCompatibility(f){
 if(!f)return;const dragged=f.drag?.active?f.groups.get(f.drag.groupId):null,selected=dragged||f.groups.get(f.selectedGroup),compatible=new Set(),dragTarget=f.drag?.active?f.drag.targetId:null;
 if(dragTarget)compatible.add(dragTarget);else if(selected&&!selected.incoming){const candidates=[...f.groups.values()].filter(g=>g.id!==selected.id&&!g.incoming&&stellarFormationOverlap(selected,g)).sort((a,b)=>Math.hypot(selected.x-a.x,selected.y-a.y)-Math.hypot(selected.x-b.x,selected.y-b.y));if(candidates[0])compatible.add(candidates[0].id)}
 for(const g of f.groups.values()){const pair=g.members.length===2,dragging=!!dragged&&g.id===dragged.id,target=dragTarget===g.id;g.fieldEl?.classList.toggle('formation-pair',pair);g.fieldEl?.classList.toggle('formation-cluster',!pair);g.fieldEl?.classList.toggle('selected',g.id===f.selectedGroup);g.fieldEl?.classList.toggle('compatible',compatible.has(g.id));g.fieldEl?.classList.toggle('formation-incoming',!!g.incoming);g.fieldEl?.classList.toggle('dragging',dragging);g.fieldEl?.classList.toggle('drag-target',target);g.fieldEl?.classList.toggle('drag-anchored',!stellarFormationGroupDraggable(g)&&!g.incoming)}
 for(const a of f.atoms.values()){const el=a.el,g=f.groups.get(a.groupId),pair=g?.members.length===2,dragging=!!dragged&&a.groupId===dragged.id,target=dragTarget===a.groupId;if(el)el.dataset.formationGroup=String(a.groupId);el?.classList.toggle('formation-pair',!!pair);el?.classList.toggle('formation-cluster',!pair);el?.classList.toggle('selected',a.groupId===f.selectedGroup);el?.classList.toggle('compatible',compatible.has(a.groupId));el?.classList.toggle('formation-incoming',!!g?.incoming);el?.classList.toggle('dragging',dragging);el?.classList.toggle('drag-target',target);el?.classList.toggle('drag-anchored',!!g&&!stellarFormationGroupDraggable(g)&&!g.incoming)}
}
...
function stellarFormationSoftSeparate(f,dt){
 const dragged=f.drag?.active?f.drag.groupId:null,groups=[...f.groups.values()].filter(g=>!g.incoming);
 for(let i=0;i<groups.length;i++)for(let j=i+1;j<groups.length;j++){
  const a=groups[i],b=groups[j];if(a.id===dragged||b.id===dragged)continue;const dx=b.x-a.x,dy=b.y-a.y,dist=Math.hypot(dx,dy)||.001,minDist=(stellarFormationVisualRadius(a,f.spec)+stellarFormationVisualRadius(b,f.spec))*.62;
  if(dist>=minDist)continue;
  const nx=dx/dist,ny=dy/dist,overlap=minDist-dist,step=Math.min(1.8,overlap*.18)*Math.min(1.8,dt/16),mass=a.members.length+b.members.length,wa=b.members.length/mass,wb=a.members.length/mass;
  a.x-=nx*step*wa;a.y-=ny*step*wa;b.x+=nx*step*wb;b.y+=ny*step*wb;
  const impulse=Math.min(.006,overlap*.00032);a.vx-=nx*impulse*wa;a.vy-=ny*impulse*wa;b.vx+=nx*impulse*wb;b.vy+=ny*impulse*wb;
 }
}
function stellarFormationRenderFrame(f,now){
 if(!f||phase().mode!=='stellarFormation')return;
 const size=starSize(),center=size/2,dt=Math.min(34,Math.max(8,now-(f.lastTime||now)));f.lastTime=now;
 for(const g of f.groups.values()){
  const dragging=f.drag?.active&&f.drag.groupId===g.id;
  if(!dragging&&!f.complete&&!f.stabilizing){
   const cap=g.incoming?.058:Math.max(.014,.05/Math.pow(g.members.length,.22)),speed=Math.hypot(g.vx,g.vy)||1;
   ...
   if(g.incoming){
    if(dist<=maxDist+5){g.incoming=false;g.fieldEl?.classList.remove('formation-incoming');for(const aid of g.members)f.atoms.get(aid)?.el?.classList.remove('formation-incoming')}
   }else if(dist>maxDist&&dist>0){
    const nx=dx/dist,ny=dy/dist,radial=g.vx*nx+g.vy*ny;
    g.x=center+nx*maxDist;g.y=center+ny*maxDist;
    if(radial>0){g.vx-=1.85*radial*nx;g.vy-=1.85*radial*ny}
   }
  }else if(!dragging){
   g.x+=(center-g.x)*.075;g.y+=(center-g.y)*.075;g.angle*=.88;if(Math.abs(g.angle)<.0005)g.angle=0;
  }
 }
 if(!f.complete&&!f.stabilizing)stellarFormationSoftSeparate(f,dt);
 for(const g of f.groups.values()){
 ...
 if(now-(f.compatibilityAt||0)>130){f.compatibilityAt=now;stellarFormationRefreshCompatibility(f)}
 f.raf=requestAnimationFrame(t=>stellarFormationRenderFrame(f,t));
}
...
function stellarFormationSelectAtom(atomId){const f=state.stellarFormation,a=f?.atoms.get(atomId);if(!f||!a||performance.now()<(a.suppressTapUntil||0)||state.locked||state.phaseDone||f.stabilizing||f.complete)return;const gid=a.groupId,g=f.groups.get(gid);if(!g||g.incoming)return;setInfoSelection({type:'preview',sym:a.sym},true);if(!f.selectedGroup){f.selectedGroup=gid;tone(310,.055,'sine',.025);stellarFormationRefreshCompatibility(f);stellarFormationUpdateHud(f);return}if(f.selectedGroup===gid){f.selectedGroup=null;tone(240,.045,'sine',.018);stellarFormationRefreshCompatibility(f);stellarFormationUpdateHud(f);return}const first=f.groups.get(f.selectedGroup);if(!first){f.selectedGroup=gid;stellarFormationRefreshCompatibility(f);return}if(!stellarFormationOverlap(first,g)){stellarFormationBlocked(g);tone(145,.07,'sine',.018);return}stellarFormationMerge(first,g)}
function stellarFormationAutoComplete(f,g){
 if(!f||f.complete||f.stabilizing||g.members.length!==f.spec.visibleTotal)return;
 if(f.inflowTimer){clearTimeout(f.inflowTimer);f.inflowTimer=null}
 f.stabilizing=true;f.selectedGroup=null;g.vx=0;g.vy=0;g.omega=0;g.angle=0;const targets=stellarFormationLayout(f.spec.total,f.spec);
 ...
 const target=targets[f.spec.total-1],id=f.nextAtomId++,atom={id,sym:'H',groupId:g.id,x:g.x+target.x,y:g.y+target.y,toLocal:{x:target.x,y:target.y},fromLocal:null,el:null},el=document.createElement('button');
...
function stellarFormationEntryPoint(f){
 const size=f.size||starSize(),center=size/2,angle=Math.random()*Math.PI*2,spawnRadius=center+stellarFormationAtomSize(f.spec)*1.35,drift=.042+Math.random()*.012,tangent=(Math.random()-.5)*.012;
 return{x:center+Math.cos(angle)*spawnRadius,y:center+Math.sin(angle)*spawnRadius,vx:-Math.cos(angle)*drift-Math.sin(angle)*tangent,vy:-Math.sin(angle)*drift+Math.cos(angle)*tangent}
}
function stellarFormationSpawnPair(f,{incoming=false}={}){
 if(!f||f.spawnedPairs>=f.spec.pairCount)return null;
 const gid=f.nextGroupId++,existing=[...f.groups.values()].map(g=>({x:g.x,y:g.y})),pt=incoming?stellarFormationEntryPoint(f):stellarFormationRandomPoint(existing,f.size,f.spec),driftAngle=Math.random()*Math.PI*2,drift=.018+Math.random()*.025,angle=Math.random()*Math.PI*2,local=stellarFormationSeedLayout(2,f.spec),velocity=incoming?pt:{vx:Math.cos(driftAngle)*drift,vy:Math.sin(driftAngle)*drift},g={id:gid,members:[],x:pt.x,y:pt.y,vx:velocity.vx,vy:velocity.vy,angle,omega:(gid%2?1:-1)*(.00022+Math.random()*.00018),reorgStart:0,fieldEl:null,incoming};
 const ca=Math.cos(angle),sa=Math.sin(angle);
 local.forEach(offset=>{const id=f.nextAtomId++,x=pt.x+offset.x*ca-offset.y*sa,y=pt.y+offset.x*sa+offset.y*ca,atom={id,sym:'H',groupId:gid,x,y,toLocal:{...offset},fromLocal:null,el:null},el=document.createElement('button');el.type='button';el.className='formation-atom formation-pair'+(incoming?' formation-incoming':'');el.dataset.formationAtom=String(id);el.style.background=elementStyle('H');el.textContent=stellarFormationElementLabel('H');el.setAttribute('aria-label',incoming?'Hidrogênio chegando do meio interestelar':'Hidrogênio em dupla H₂');el.addEventListener('pointerdown',ev=>armStellarFormationDrag(id,el,ev));el.addEventListener('pointermove',ev=>moveStellarFormationDrag(id,ev));el.addEventListener('pointerup',ev=>finishStellarFormationDrag(id,ev,false));el.addEventListener('pointercancel',ev=>finishStellarFormationDrag(id,ev,true));el.addEventListener('click',ev=>{ev.preventDefault();ev.stopPropagation();stellarFormationSelectAtom(id)});atom.el=el;f.atomsLayer.appendChild(el);f.atoms.set(id,atom);g.members.push(id)});
 f.groups.set(g.id,g);stellarFormationMakeField(f,g);f.spawnedPairs++;stellarFormationRefreshCompatibility(f);return g
}
function stellarFormationScheduleSupply(f,delay=620){
 if(!f||f.complete||f.stabilizing||f.inflowTimer||f.spawnedPairs>=f.spec.pairCount||f.groups.size>=f.spec.maxConcurrentGroups)return;
 f.inflowTimer=setTimeout(()=>{
  f.inflowTimer=null;if(state.stellarFormation!==f||f.complete||f.stabilizing)return;
  if(f.spawnedPairs<f.spec.pairCount&&f.groups.size<f.spec.maxConcurrentGroups){stellarFormationSpawnPair(f,{incoming:true});stellarFormationUpdateHud(f);tone(190,.05,'sine',.012)}
  if(f.spawnedPairs<f.spec.pairCount&&f.groups.size<f.spec.maxConcurrentGroups)stellarFormationScheduleSupply(f,460+Math.random()*220);
 },delay)
}
function stellarFormationMerge(a,b){
 const f=state.stellarFormation;if(!f||!f.groups.has(a.id)||!f.groups.has(b.id)||a.incoming||b.incoming)return;
 const members=[...a.members,...b.members],mass=members.length,cx=(a.x*a.members.length+b.x*b.members.length)/mass,cy=(a.y*a.members.length+b.y*b.members.length)/mass,newId=f.nextGroupId++,vx=(a.vx*a.members.length+b.vx*b.members.length)/mass,vy=(a.vy*a.members.length+b.vy*b.members.length)/mass,g={id:newId,members,x:cx,y:cy,vx,vy,angle:0,omega:(newId%2?1:-1)*(.00017+Math.random()*.00015),reorgStart:performance.now(),fieldEl:null,incoming:false};
 const targets=stellarFormationLayout(mass);
 members.forEach((aid,i)=>{const atom=f.atoms.get(aid);atom.groupId=newId;atom.fromLocal={x:(atom.x??cx)-cx,y:(atom.y??cy)-cy};atom.toLocal={x:targets[i].x,y:targets[i].y}});
 a.fieldEl?.remove();b.fieldEl?.remove();f.groups.delete(a.id);f.groups.delete(b.id);f.groups.set(newId,g);stellarFormationMakeField(f,g);f.selectedGroup=null;recordFlow(1);tone(260+Math.min(360,mass*5),.12,'triangle',.032);vibrate(6);stellarFormationRefreshCompatibility(f);stellarFormationUpdateHud(f);
 if(mass===f.spec.visibleTotal)stellarFormationAutoComplete(f,g);else stellarFormationScheduleSupply(f,520+Math.random()*260);
}
function stellarFormationRandomPoint(existing,size,spec=stellarFormationSpec()){
 const center=size/2,pad=Math.max(32,size*.075),maxR=Math.max(30,center-pad),minGap=Math.max(46,Math.min(72,stellarFormationAtomSize(spec)*1.65));
 for(let tries=0;tries<320;tries++){
  const angle=Math.random()*Math.PI*2,radius=maxR*Math.sqrt(Math.random()),x=center+Math.cos(angle)*radius,y=center+Math.sin(angle)*radius;
  if(existing.every(p=>Math.hypot(p.x-x,p.y-y)>minGap))return{x,y};
 }
 const angle=existing.length*2.399963229728653,radius=maxR*Math.sqrt((existing.length+.5)/Math.max(1,spec.initialGroups));
...
function startStellarFormationStage(){
 ...
 layer.className='stellar-formation-layer';layer.setAttribute('aria-label',`${spec.pairCount} duplas de Hidrogênio chegando gradualmente e formando um hexágono com ${stellarFormationLayerDescription(spec)}`);layer.style.setProperty('--formationAtomSize',stellarFormationAtomSize(spec)+'px');fields.className='formation-fields';atomsLayer.className='formation-atoms';layer.append(fields,atomsLayer);dom.star.appendChild(layer);dom.star.classList.add('stellar-formation-mode');
 const f={layer,fields,atomsLayer,atoms:new Map(),groups:new Map(),selectedGroup:null,drag:null,nextGroupId:1,nextAtomId:1,spawnedPairs:0,inflowTimer:null,raf:0,lastTime:performance.now(),compatibilityAt:0,stabilizing:false,complete:false,size,spec};state.stellarFormation=f;
 for(let i=0;i<spec.initialGroups;i++)stellarFormationSpawnPair(f,{incoming:false});
 stellarFormationUpdateHud(f);stellarFormationRefreshCompatibility(f);f.raf=requestAnimationFrame(t=>stellarFormationRenderFrame(f,t));if(phase().id==='first_generation_formation')setTimeout(()=>{if(state.stellarFormation===f&&!state.phaseDone)toast('Arraste um grupo até outro — ou toque nos dois.')},650);
}
...
function objectiveSatisfied(s=phase()){
 ...
 if(s.mode==='stellarFormation')return !!state.stellarFormationCompleted||!!state.stellarFormation?.complete;
 if(s.coronalJetTutorial)return(state.coronalJetCount||0)>=s.target;if(s.mode==='convection')return(state.convectionMoves||0)>=s.target;
...
function ensurePhaseCompletionShade(){let shade=document.getElementById('phaseCompletionShade');if(shade)return shade;shade=document.createElement('div');shade.id='phaseCompletionShade';shade.className='phase-completion-shade';shade.setAttribute('aria-hidden','true');dom.star.appendChild(shade);return shade}
function setPhaseCompletionPresentation(active){const shade=ensurePhaseCompletionShade();dom.star.classList.toggle('completion-ready',!!active);shade.classList.toggle('show',!!active)}
function freezeCompletedPhaseInteraction(){
 state.locked=true;state.selected=[];state.freeSelected=[];state.primordialSelected=null;state.selectedNeutron=null;state.selectedCosmic=null;state.blackHoleSelected=false;state.convectionArmed=false;state.convectionConfirmPending=false;
 stopPrimordialDrift();stopPrimordialMoleculeDrift();cancelParticleDrag();cancelPrimordialFreeDrag();cancelStellarBoardDrag();stopAccretionFeed();
 if(state.neutronTimer){clearInterval(state.neutronTimer);state.neutronTimer=null}if(state.neutronTick){clearInterval(state.neutronTick);state.neutronTick=null}if(state.cosmicTimer){clearTimeout(state.cosmicTimer);state.cosmicTimer=null}if(state.solarWindCheckTimer){clearTimeout(state.solarWindCheckTimer);state.solarWindCheckTimer=null}
}
function checkComplete(){
 ...
 if(s.mode==='stellarFormation'&&!state.stellarFormationCompleted){const f=state.stellarFormation;if(!f?.complete||f.completionMaterializing)return;f.completionMaterializing=true;state.locked=true;stellarFormationAdvance({unlock:false}).then(()=>{if(phase()===s&&!state.phaseDone){state.locked=false;checkComplete()}}).catch(err=>{console.error(err);if(phase()===s)state.locked=false});return}
 state.readyToAdvance=true;freezeCompletedPhaseInteraction();if(s.mode==='stellarFormation')dom.star.parentElement?.classList.add('formation-ready-shell');save();$('phaseEndBtn').classList.remove('show');dom.star.classList.add('critical');setPhaseCompletionPresentation(true);phaseCompletionReward(s);setTimeout(()=>{if(phase()===s&&state.readyToAdvance)$('phaseEndBtn').classList.add('show')},260);
 ...
 else if(s.id==='solar_wind')announce('OBJETIVO CONCLUÍDO','VENTO SOLAR','A ionização produziu plasma livre e o excesso de partículas escapou da coroa.');
...
async function advanceStellarAtomicPhase(){
...
function startPhase(index,announcePhase=true,forcePopup=false){state.phaseIndex=Math.max(0,Math.min(PHASES.length-1,index));const s=phase();setPhaseCompletionPresentation(false);document.documentElement.dataset.arduaEnginePhase=s.id;window.dispatchEvent(new CustomEvent('ardua:engine-phase',{detail:{id:s.id}}));const captureIndex=phaseIndexById.get('proton_capture')??Infinity,neutronCaptureIndex=phaseIndexById.get('primordial_t')??Infinity,redIndex=phaseIndexById.get('he_red')??1,preserved=s.mode==='blackhole'&&Array.isArray(state.collapseMatterSnapshot)&&state.collapseMatterSnapshot.length?[...state.collapseMatterSnapshot]:null;if(state.phaseIndex>redIndex)state.ignited=true;if(state.phaseIndex>=captureIndex)state.protonCaptureUnlocked=true;if(state.phaseIndex>=neutronCaptureIndex)state.neutronCaptureUnlocked=true;state.phaseDone=false;state.readyToAdvance=false;state.boardDrag=null;state.flow=0;state.flowMilestones=new Set();resetChainFeedback();state.locked=false;state.popupOpen=false;state.popupKind=null;state.lessonResolver=null;state.phaseMilestoneAnnounced=false;state.created={};state.atomicNeutralCreated={};state.stellarFormationCompleted=false;state.stellarIonizations=0;state.stellarRecombinations=0;state.stellarIonizedSpecies=new Set();state.stellarRecombinedSpecies=new Set();state.solarWindEvents=0;state.solarWindRunning=false;if(state.solarWindCheckTimer){clearTimeout(state.solarWindCheckTimer);state.solarWindCheckTimer=null}state.nextMatterOrigin=1;state.objectiveLineages=new Set();state.protonCaptures=0;state.protonCaptureProducts={};state.protonCaptureAttempts={};state.rpIonized=0;state.rpPhotoReturns=0;state.rpCyclesObserved=0;state.rpWaitDecays=0;state.neutronSourceActivations=0;state.neutronPulsesObserved=0;state.neutronBranchesObserved=0;state.neutronBetaWaits=0;state.neutronFreezeouts=0;state.neutronStormsObserved=0;state.fusionAttempts={};state.atlasProgress=0;state.atlasAttempts={};state.atlasBarrierPassed={};state.atlasPhaseTooltipSeen=false;state.coulombRepulsions=0;state.convectionCharge=0;state.convectionArmed=false;state.convectionConfirmPending=false;state.convectionPathCells=[];state.convectionMoves=0;state.convectionLessonShown=false;state.coronalJetCount=0;state.coronalJetRunning=false;state.neutronBirths=0;state.primordialDByProton=0;state.primordialDByNeutron=0;state.nuclearRound=0;state.crushed=0;state.absorbed=0;state.postInitialMatter=0;state.explosiveHits=0;state.decayFound=new Set();state.postHoldLearned=false;state.blackHoleSelected=false;state.radioactiveProofDone=false;state.selected=[];state.contextRecipeKey=null;state.infoSelection=null;state.stratificationCoreGroup=null;state.objectiveMotifCombinationOctave=0;if(state.tooltipOpen){const tip=$('eventTooltip');tip?.classList.remove('show');tip?.setAttribute('aria-hidden','true');state.tooltipOpen=false;state.tooltipResolver=null;dom.star.classList.remove('event-paused')}stopPrimordialDrift();stopAccretionFeed();stopCosmicRaySystem();if(state.crushTimer)clearTimeout(state.crushTimer);if(state.coreHoldTimer)clearTimeout(state.coreHoldTimer);state.crushTimer=null;state.crushId=null;state.coreHoldTimer=null;dom.remnantCore?.classList.remove('core-hold');dom.star.classList.remove('core-collapsing');stopNeutronSystem();$('stellarIntro').classList.remove('show');$('phaseEndBtn').classList.toggle('show',state.readyToAdvance);$('explosion').innerHTML='';dom.pieces.classList.remove('hidden');removeStellarFormation();dom.star.classList.remove('critical','phase-active','neutron-active','milestone-flash','ignition-flash','remnant-mode','blackhole-mode','post-active','primordial-transition','spallation-mode','decay-mode','electron-network','white-electron-network','white-structure','cumulative-shells');applyGeometry();if(!stellarAtomicTransferTarget(s)&&state.stellarAtomicTransfer)state.stellarAtomicTransfer=null;if(preserved){restoreMatterSnapshot(preserved);state.collapseMatterSnapshot=null}else{if(s.mode!=='blackhole')state.collapseMatterSnapshot=null;fillStage()}prepareCumulativeStellarAtomicMatter(s);if(s.mode==='stellarFormation')startStellarFormationStage();if(s.mode==='blackhole')state.postInitialMatter=state.pieces.size;ensureCumulativeParticleFuel(s);const popupShown=showStellarPopup(forcePopup);if(!popupShown&&['neutron','neutronize'].includes(s.mode))setTimeout(startNeutronSystem,420);if(!popupShown&&s.mode==='accretion')startAccretionFeed();if(!popupShown&&['spallation','neutrino','gamma'].includes(s.mode))startCosmicRaySystem();if(announcePhase&&!popupShown)announce(s.branch,s.title,s.mode==='showcase'?s.meta:(s.mode==='neutronize'?s.meta:s.meta||`Novo: ${E[s.new].name}`));save();render();if(s.mode==='campaignMilestone')setTimeout(checkComplete,80)}
function modalPrimaryLine(s=phase()){
 if(s.coronalJetTutorial)return 'Convecção + íon superficial → reconexão magnética → jato coronal';
 ...
 if(s.id==='first_nebulae')return 'HeH⁺ + H → H₂';
 if(s.id==='brown')return '²H + H → ³He + γ';
...
function modalSecondaryLine(s=phase()){
 if(s.coronalJetTutorial)return 'Ejete o íon da superfície; depois a linha convectiva se reorganiza';
 ...
 if(s.id==='first_atomic_bonds')return 'Selecione um Hélio e um Hidrogênio neutros para formar uma dupla ligada';
...
function phaseIntroTitle(s=phase(),base={}){
 const special={primordial_d:'PRIMEIRO NÚCLEO',primordial_t:'TRÍTIO',primordial_he3:'HÉLIO-3',primordial_he3d:'HÉLIO-4',primordial_td:'HÉLIO-4',primordial_li:'LÍTIO-7',atomic_he:'HÉLIO',atomic_h:'HIDROGÊNIO',atomic_li:'LÍTIO',solar_wind:'VENTO SOLAR',stellar_ionization:'IONIZAÇÃO ESTELAR',stellar_recombination:'RECOMBINAÇÃO ESTELAR',stellar_movement:'MOVIMENTAÇÃO ESTELAR',coulomb_intro:'BARREIRA DE COULOMB',stellar_convection:'CONVECÇÃO ESTELAR',stellar_li:'LÍTIO ESTELAR'};
...
function phaseFamily(p){
 ...
 if(['brown','he_red','stellar_movement','he_orange','he_yellow','solar_wind','stellar_ionization','stellar_recombination','coulomb_intro','stellar_convection','stellar_li','fragile','c','n','o'].includes(p.id))return 'Estrelas de baixa e média massa';
