...
const PHASES=[
 ...
 {id:'first_generation_formation',branch:'Nascimento estelar · Primeira Geração',title:'Formação da Primeira Geração',meta:'Reúna as moléculas pela gravidade',new:'H',mode:'stellarFormation',formationLayers:4,target:37,visual:'nebula',fill:0,endEvent:'stellarBirth',endLabel:'NASCE UMA<br>NOVA ESTRELA',menuTag:'37 H',durationClass:'long'},
 {id:'brown_formation',branch:'Nascimento estelar · massa muito baixa',title:'Formação da Anã Marrom',meta:'Reúna as moléculas pela gravidade',new:'H',mode:'stellarFormation',formationLayers:2,target:7,visual:'nebula',fill:0,endEvent:'stellarBirth',endLabel:'FORMAR<br>ANÃ MARROM',menuTag:'7 H',durationClass:'quick'},
 ...
 {id:'low_mass_formation',branch:'Nascimento estelar · baixa massa',title:'Formação da Estrela de Baixa Massa',meta:'Reúna as moléculas pela gravidade',new:'H',mode:'stellarFormation',formationLayers:2,target:19,visual:'nebula',fill:0,endEvent:'stellarBirth',endLabel:'FORMAR<br>ANÃ VERMELHA',menuTag:'19 H',durationClass:'quick'},
...
{id:'stellar_recombination',branch:'Coroa estelar · recombinação',title:'Recombinação Estelar',meta:'Íon⁺ + e⁻ → Átomo + γ',new:'H',mode:'stellarRecombination',target:3,visual:'redDwarf',fill:15,pool:['H','H','H','H','H','He'],recombinationSpecies:['H','He','Li'],menuTag:'RECOMBINAÇÃO',endLabel:'ESPALHAR<br>POEIRA ESTELAR'},
 {id:'intermediate_mass_formation',branch:'Nascimento estelar · massa intermediária',title:'Formação da Estrela de Massa Intermediária',meta:'Reúna as moléculas pela gravidade',new:'H',mode:'stellarFormation',formationLayers:3,target:37,visual:'nebula',fill:0,endEvent:'stellarBirth',endLabel:'FORMAR<br>ANÃ LARANJA',menuTag:'37 H',durationClass:'short'},
...
{id:'coronal_jets',branch:'Gigante vermelha · atividade magnética',title:'Jatos Coronais',meta:'Convecção + íon superficial → reconexão magnética → jato coronal',new:'He',mode:'convection',target:2,visual:'redGiant',fill:34,pool:['H','H','He','He3'],gravityDelay:142,fusionTempMax:1.2e8,reuseFusion:true,objectiveOnlyProgress:true,coronalJetTutorial:true,menuTag:'JATOS CORONAIS',endLabel:'ESPALHAR<br>POEIRA ESTELAR'},
 ...
 {id:'giant_formation',branch:'Nascimento estelar · bifurcação das gigantes',title:'Formação das Gigantes',meta:'Reúna as moléculas pela gravidade',new:'H',mode:'stellarFormation',formationLayers:4,target:61,visual:'nebula',fill:0,endEvent:'stellarBirth',endLabel:'ESCOLHER<br>GIGANTE',menuTag:'61 H',durationClass:'long'},
 ...
 {id:'high_mass_formation',branch:'Nascimento estelar · alta massa',title:'Formação da Estrela de Alta Massa',meta:'Reúna as moléculas pela gravidade',new:'H',mode:'stellarFormation',formationLayers:5,target:91,visual:'nebula',fill:0,endEvent:'stellarBirth',endLabel:'INICIAR<br>QUEIMA DE CARBONO',menuTag:'91 H',durationClass:'epic'},
...
function spawnFloatingParticle(kind,x=null,y=null,fromOutside=false){
 ...
 setTimeout(()=>{const q=state.primordialParticles.get(p.id);if(q){q.reacting=false;q.incoming=false;renderPrimordialParticles();startPrimordialDrift()}},fromOutside?900:420);return p
}
function ensurePrimordialParticleMix({p=0,e=0,n=0}={}){for(const [kind,min] of Object.entries({p,e,n}))while(countFloatingParticle(kind)<min)spawnFloatingParticle(kind,null,null,true)}
...
function primordialGoalCount(s=phase()){if(isPrimordial(s)&&s.mode==='atomicRecombination'){let total=0;state.pieces.forEach(p=>{if(p.free&&p.sym===s.atomicTarget&&p.matterState==='atom'&&pieceCharge(p)===0)total++});return total}return state.created[s.new]||0}
...
function pieceCanBindElectron(p){const s=phase(),currentLesson=s?.mode==='atomicRecombination'&&p?.sym===s.atomicTarget;return !!p&&p.free&&['H','He','Li'].includes(p.sym)&&(currentLesson||atomicRecombinationLearned(p.sym))&&pieceCharge(p)>0}
...
function ensureFreshPrimordialParticle(kind){
 const existing=[...state.primordialParticles.values()].filter(p=>p.kind===kind);
 if(kind!=='n'||primordialNeutronsStable()){if(!existing.length)spawnFloatingParticle(kind,null,null,true);return}
 const fresh=existing.some(p=>(p.lifetimeRounds||3)-(state.nuclearRound-(p.bornRound??state.nuclearRound))>=3);
 if(!fresh)spawnFloatingParticle('n',null,null,true);
}
...
function ensureCumulativeParticleFuel(s=phase()){
 ...
 if(s?.coronalJetTutorial){ensurePrimordialParticleMix({p:1,n:1,e:1});startPrimordialDrift();return}
...
function renderPrimordialParticles(){
 if(!dom.primordial)return;dom.primordial.classList.toggle('free-drag-active',!!state.freeDrag?.active);const s=phase(),primordialActive=isPrimordial(s)&&s.mode!=='opening',canUseCumulative=cumulativeParticleInteractionAllowed(s),canUseStellarProton=!!stellarProtonRecipe(s)||protonCaptureAvailable(s),canUseStellarAtomic=stellarAtomicChemistryAllowed(s),dragParticle=state.particleDrag?.active?state.primordialParticles.get(state.particleDrag.id):null,selectedParticle=dragParticle||(state.primordialSelected!==null?state.primordialParticles.get(state.primordialSelected):null),dragSourcePiece=state.freeDrag?.active&&!state.freeDrag.moleculeId?state.pieces.get(state.freeDrag.sourceId):null,selectedPiece=dragSourcePiece||(state.freeSelected.length?state.pieces.get(state.freeSelected[0]):null),selectedBoardPiece=state.selected.length?state.pieces.get(state.board[state.selected[0]]):null;dom.primordial.classList.toggle('active',(primordialActive||canUseStellarProton||canUseCumulative||canUseStellarAtomic)&&!state.locked);const existing=new Map([...dom.primordial.querySelectorAll('.primordial-particle')].map(el=>[+el.dataset.id,el]));
 ...
 state.primordialParticles.forEach((p,id)=>{let el=existing.get(id);if(!el){el=document.createElement('button');el.type='button';el.dataset.id=id;el.addEventListener('contextmenu',ev=>ev.preventDefault());el.addEventListener('pointerdown',ev=>armParticleDrag(id,el,ev));el.addEventListener('pointermove',ev=>moveParticleDrag(id,ev));el.addEventListener('pointerup',ev=>finishParticleDrag(id,ev,false));el.addEventListener('pointercancel',ev=>finishParticleDrag(id,ev,true));el.addEventListener('click',ev=>{ev.preventDefault();ev.stopPropagation();const q=state.primordialParticles.get(id);if(q&&performance.now()<(q.suppressTapUntil||0))return;tapPrimordialParticle(id)});dom.primordial.appendChild(el)}const cls=p.kind==='p'?'proton':p.kind==='pos'?'positron':p.kind==='n'?'neutronfree':'electron',label=p.kind==='p'?'+':p.kind==='pos'?'e⁺':p.kind==='n'?'n':'−';let candidate=false,interactive=false;
   ...
   const reserve=!isPrimordial(s)&&!stellarAtomicMode(s)&&['p','n','e'].includes(p.kind)&&!p.reacting&&!foregroundFree.has(id),freeDragSource=state.freeDrag?.active&&!state.freeDrag.moleculeId?state.pieces.get(state.freeDrag.sourceId):null,freeDragCompatible=!!freeDragSource&&!p.reacting&&Math.hypot(freeDragSource.x-p.x,freeDragSource.y-p.y)<=64&&((p.kind==='e'&&pieceCanBindElectron(freeDragSource))||!!primordialMixedReaction(freeDragSource.sym,p.kind)),freeDragTarget=state.freeDrag?.active&&state.freeDrag.target?.type==='particle'&&state.freeDrag.target.id===id,dropTarget=(state.particleDrag?.active&&state.particleDrag.target?.type==='particle'&&state.particleDrag.target.id===id)||freeDragTarget||freeDragCompatible;el.className=`primordial-particle ${cls}${p.kind==='n'&&p.unstable?' unstable':''}${state.primordialSelected===id?' selected':''}${candidate?' candidate':''}${dropTarget?' drop-target':''}${p.reacting?' reacting':''}${p.dragging?' dragging':''}${p.throwing?' throwing':''}${p.coulombDeflect?' coulomb-deflect':''}${p.tunneling?' tunneling':''}${reserve?' particle-reserve':''}`;el.textContent=label;el.style.pointerEvents=reserve?'none':'auto';el.setAttribute('aria-hidden',reserve?'true':'false');el.dataset.mechanical=interactive&&!reserve?'1':'0';el.style.left=p.x+'px';el.style.top=p.y+'px';existing.delete(id)});existing.forEach(el=>el.remove())
}
...
function primordialFreeDragTarget(d){
 ...
 for(const particle of state.primordialParticles.values()){
  if(particle.reacting||particle.dragging)continue;
  const bind=particle.kind==='e'&&pieceCanBindElectron(source),reaction=primordialMixedReaction(source.sym,particle.kind);if(!bind&&!reaction)continue;
  const dist=Math.hypot(source.x-particle.x,source.y-particle.y);if(dist<=58&&dist<bestDist){bestDist=dist;best={type:'particle',id:particle.id,pieceIds:[],mode:bind?'bindElectron':'mixed'}}
 }
 return best
}
function primordialFreePointerTarget(d,ev){
 if(!d?.active||d.moleculeId||!ev)return null;const source=state.pieces.get(d.sourceId);if(!source)return null;
 let hit=null,best=Infinity;for(const el of dom.primordial.querySelectorAll('.primordial-particle[data-id]')){const particle=state.primordialParticles.get(Number(el.dataset.id));if(!particle||particle.reacting||particle.dragging)continue;const bind=particle.kind==='e'&&pieceCanBindElectron(source),reaction=primordialMixedReaction(source.sym,particle.kind);if(!bind&&!reaction)continue;const r=el.getBoundingClientRect(),cx=r.left+r.width/2,cy=r.top+r.height/2,dist=Math.hypot(ev.clientX-cx,ev.clientY-cy),radius=Math.max(30,Math.max(r.width,r.height)*.75);if(dist<=radius&&dist<best){best=dist;hit={particle,mode:bind?'bindElectron':'mixed'}}}
 return hit?{type:'particle',id:hit.particle.id,pieceIds:[],mode:hit.mode}:null
}
...
function updateHUD(){
 ...
 const formationSpec=s.mode==='stellarFormation'?stellarFormationSpec(s):null,formationLargest=s.mode==='stellarFormation'?(state.stellarFormationComplete?formationSpec.total:(state.stellarFormation?Math.max(0,...[...state.stellarFormation.groups.values()].map(g=>g.members.length)):0)):0;
 ...
 $('phaseEndBtn').innerHTML=phaseEndButtonLabel(s);updateObjective();applyVisual();applyRewardProgressVisuals();renderInfoPanel()
}
function updateObjective(){
 const s=phase();
 if(state.readyToAdvance){$('goalText').textContent='Fase concluída com sucesso';setFormula('');return}
...
function startStellarFormationStage(){
 ...
 state.stellarFormationComplete=false;const f={layer,fields,atomsLayer,atoms:new Map(),groups:new Map(),selectedGroup:null,drag:null,nextGroupId:1,nextAtomId:1,spawnedPairs:0,inflowTimer:null,raf:0,lastTime:performance.now(),compatibilityAt:0,stabilizing:false,complete:false,size,spec};state.stellarFormation=f;
...
function objectiveSatisfied(s=phase()){
 ...
 if(s.mode==='stellarFormation')return !!state.stellarFormationComplete;
...
function activatePhaseCompletionGate(s=phase()){
 if(phase()!==s||!state.readyToAdvance||state.phaseDone)return false;
 state.locked=true;state.selected=[];state.freeSelected=[];state.primordialSelected=null;state.selectedNeutron=null;state.selectedCosmic=null;state.convectionArmed=false;state.convectionConfirmPending=false;cancelParticleDrag();cancelPrimordialFreeDrag();dom.star.classList.add('phase-ready');$('phaseEndBtn').classList.add('show');render();return true
}
function checkComplete(){
 ...
 state.readyToAdvance=true;state.selected=[];save();$('phaseEndBtn').classList.remove('show');dom.star.classList.add('critical');phaseCompletionReward(s);render();setTimeout(()=>activatePhaseCompletionGate(s),720);
...
async function advanceStellarFormationPhase(){if(!state.phaseDone)return;$('phaseEndBtn').classList.remove('show');state.locked=true;dom.star.classList.add('primordial-transition');tone(300,.16,'sine',.024);await wait(330);dom.star.classList.remove('primordial-transition');advancePhase()}
...
async function endPhaseAction(){
 ...
 state.phaseDone=true;state.locked=true;$('phaseEndBtn').classList.remove('show');dom.star.classList.remove('phase-ready');stopPrimordialDrift();cancelParticleDrag();stopAccretionFeed();stopCosmicRaySystem();stopNeutronSystem();
 if(phase()!==s)return;
 if(isPrimordial(s))return advancePrimordial();if(s.endEvent==='plasmaTransition')return advanceStellarAtomicPhase();if(s.endEvent==='stellarBirth')return advanceStellarFormationPhase();if(s.endEvent==='finale')return finishCampaign();if(s.endEvent==='postTransition')return compactAdvance();return scatterStage()
}
...
function modalPrimaryLine(s=phase()){
 ...
 if(s.mode==='stellarFormation')return 'Reúna as moléculas pela gravidade';
