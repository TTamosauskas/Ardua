...
function renderPieces(){
 ...
 state.pieces.forEach((p,id)=>{
  ...
  if(!el){
   ...
   el.addEventListener('pointerdown',ev=>{if(['neutronize','neutron','guidedDecay','decayGarden'].includes(phase().mode)||(isPostAtomMode()&&phase().mode!=='blackhole')){ev.stopPropagation();beginCrushHold(id,el,ev)}armPrimordialFreeDrag(id,el,ev)});
   el.addEventListener('pointermove',ev=>movePrimordialFreeDrag(id,ev));
   el.addEventListener('pointerup',ev=>{finishPrimordialFreeDrag(id,ev,false);cancelCrushHold(id)});el.addEventListener('pointercancel',ev=>{finishPrimordialFreeDrag(id,ev,true);cancelCrushHold(id)});el.addEventListener('pointerleave',ev=>{if(ev.pointerType==='mouse')cancelCrushHold(id)});
  ...
  el.className='atom'+matterClass+(s.coronalJetTutorial?' coronal-uniform':'')+(p.sym==='Plus'?' proton-piece':'')+(selected?' selected':'')+(partner&&!selected?' candidate':'')+(decayReady?' decay-ready':'')+(pieceIsUnstable(p)?' unstable':'')+(p.neutronBetaPending?' beta-waiting':'')+(p.longRadioactive?' long-radioactive':'')+(p.radioactiveReady?' radioactive-proof':'')+(p.compacted?' compacted':'')+(p.atlasCompound?' atlas-compound':'')+(p.atlasRebound?' atlas-rebound':'')+(p.newborn?' newborn':'')+(p.convecting?' convecting':'')+(freeDragging?' primordial-dragging':'')+(dropTarget?' drop-target':'')+(state.convectionArmed&&!state.convectionConfirmPending&&!p.free?' convection-choice':'')+((state.convectionPathCells||[]).includes(p.cell)?' convection-path':'');el.style.left=p.x+'px';el.style.top=p.y+'px';if(primordial&&p.free){el.style.setProperty('--floatDelay',`${-((id%19)*.17)}s`)}else el.style.removeProperty('--floatDelay');existing.delete(id)
