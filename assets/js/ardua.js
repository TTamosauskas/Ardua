...
function configureRelaxedFlow(phases){
 ...
 set('thermal_ionization',3,3);set('he_red',6,16);set('he_orange',5,20);set('he_yellow',6,24);set('coulomb_intro',3,9);set('stellar_li',4,12);
...
function configurePhaseDurationBudget(phases){
 ...
 setClass(['atomic_li','first_atomic_bonds','first_nebulae','brown','he_red','he_orange','he_yellow','solar_wind','stellar_ionization','thermal_ionization','stellar_recombination','coulomb_intro','stellar_convection'],'short');
 ...
 for(const p of phases){
   ...
   if(['guidedDecay','reactionExplore','spallation','neutrino','gamma','protonCapture','thermalIonization','convection','neutronize','remnant','pulsar','accretion','collapseFinal','blackhole'].includes(p.mode)){
...
function stellarAtomicMode(s=phase()){return !!s&&['stellarIonization','thermalIonization','stellarRecombination'].includes(s.mode)}
...
function atomicMovementAllowed(s=phase()){return movementMechanicUnlocked(s)||thermalCoreMovementAllowed(s)}
...
function canSelectAtomForMovement(p,s=phase()){return !!p&&!p.free&&!p.thermalHeating&&p.cell!==null&&p.cell!==undefined&&movableEmptyNeighbors(p.cell,s).length>0}
...
function renderPieces(){
 ...
 state.pieces.forEach((p,id)=>{
  ...
  el.className='atom'+matterClass+(s.coronalJetTutorial?' coronal-uniform':'')+(p.sym==='Plus'?' proton-piece':'')+(selected?' selected':'')+(partner&&!selected?' candidate':'')+(decayReady?' decay-ready':'')+(pieceIsUnstable(p)?' unstable':'')+(p.neutronBetaPending?' beta-waiting':'')+(p.longRadioactive?' long-radioactive':'')+(p.radioactiveReady?' radioactive-proof':'')+(p.compacted?' compacted':'')+(p.atlasCompound?' atlas-compound':'')+(p.atlasRebound?' atlas-rebound':'')+(p.newborn?' newborn':'')+(p.thermalHeating?' thermal-heating':'')+(p.convecting?' convecting':'')+(state.convectionArmed&&!state.convectionConfirmPending&&!p.free?' convection-choice':'')+((state.convectionPathCells||[]).includes(p.cell)?' convection-path':'');el.style.left=p.x+'px';el.style.top=p.y+'px';if(primordial&&p.free){el.style.setProperty('--floatDelay',`${-((id%19)*.17)}s`)}else el.style.removeProperty('--floatDelay');existing.delete(id)
...
function ensureOpportunity(){
  const s=phase();prepareCumulativeStellarAtomicMatter(s);ensureCumulativeThermalAtom(s);ensureCumulativeParticleFuel(s);if(ensureNeutronMechanicOpportunity(s))return;
...
function stellarFormationSelectAtom(atomId){const f=state.stellarFormation,a=f?.atoms.get(atomId);if(!f||!a||state.locked||state.phaseDone||f.stabilizing||f.complete)return;const gid=a.groupId,g=f.groups.get(gid);if(!g)return;setInfoSelection({type:'preview',sym:a.sym},true);if(!f.selectedGroup){f.selectedGroup=gid;tone(310,.055,'sine',.025);stellarFormationRefreshCompatibility(f);stellarFormationUpdateHud(f);return}if(f.selectedGroup===gid){f.selectedGroup=null;tone(240,.045,'sine',.018);stellarFormationRefreshCompatibility(f);stellarFormationUpdateHud(f);return}const first=f.groups.get(f.selectedGroup);if(!first){f.selectedGroup=gid;stellarFormationRefreshCompatibility(f);return}if(!stellarFormationOverlap(first,g)){stellarFormationBlocked(g);tone(145,.07,'sine',.018);return}stellarFormationMerge(first,g)}
...
function stellarFormationMerge(a,b){
 const f=state.stellarFormation;if(!f||!f.groups.has(a.id)||!f.groups.has(b.id))return;
 const members=[...a.members,...b.members],mass=members.length,cx=(a.x*a.members.length+b.x*b.members.length)/mass,cy=(a.y*a.members.length+b.y*b.members.length)/mass,newId=f.nextGroupId++,vx=(a.vx*a.members.length+b.vx*b.members.length)/mass,vy=(a.vy*a.members.length+b.vy*b.members.length)/mass,g={id:newId,members,x:cx,y:cy,vx,vy,angle:0,omega:(newId%2?1:-1)*(.00017+Math.random()*.00015),reorgStart:performance.now(),fieldEl:null};
 ...
 if(mass===f.spec.visibleTotal)stellarFormationAutoComplete(f,g);
}
...
function startStellarFormationStage(){
 ...
 layer.className='stellar-formation-layer';layer.setAttribute('aria-label',`${spec.pairCount} duplas de Hidrogênio formando um hexágono com ${stellarFormationLayerDescription(spec)}`);layer.style.setProperty('--formationAtomSize',stellarFormationAtomSize(spec)+'px');fields.className='formation-fields';atomsLayer.className='formation-atoms';layer.append(fields,atomsLayer);dom.star.appendChild(layer);dom.star.classList.add('stellar-formation-mode');
...
function modalPrimaryLine(s=phase()){
 ...
 if(s.mode==='thermalIonization')return 'Leve um átomo ao núcleo.';
...
function modalSecondaryLine(s=phase()){
 ...
 if(s.mode==='thermalIonization')return 'No centro, o átomo aquece por uma rodada e então perde todos os elétrons ligados';
...
function phaseIntroTitle(s=phase(),base={}){
 const special={primordial_d:'PRIMEIRO NÚCLEO',primordial_t:'TRÍTIO',primordial_he3:'HÉLIO-3',primordial_he3d:'HÉLIO-4',primordial_td:'HÉLIO-4',primordial_li:'LÍTIO-7',atomic_he:'HÉLIO',atomic_h:'HIDROGÊNIO',atomic_li:'LÍTIO',solar_wind:'VENTO SOLAR',stellar_ionization:'IONIZAÇÃO ESTELAR',thermal_ionization:'IONIZAÇÃO TÉRMICA',stellar_recombination:'RECOMBINAÇÃO ESTELAR',stellar_movement:'MOVIMENTAÇÃO ESTELAR',coulomb_intro:'BARREIRA DE COULOMB',stellar_convection:'CONVECÇÃO ESTELAR',stellar_li:'LÍTIO ESTELAR'};
...
function phaseFamily(p){
 ...
 if(['brown','he_red','stellar_movement','he_orange','he_yellow','solar_wind','stellar_ionization','thermal_ionization','stellar_recombination','coulomb_intro','stellar_convection','stellar_li','fragile','c','n','o'].includes(p.id))return 'Estrelas de baixa e média massa';
