...
const NS='http://www.w3.org/2000/svg';let frame=0;
...
function add(from,to,cls='compact',bend=.5){const a=center(from,'bottom'),b=center(to,'top');if(!a||!b)return;const mid=a.y+(b.y-a.y)*bend,p=document.createElementNS(NS,'path');p.setAttribute('d',`M ${a.x.toFixed(1)} ${a.y.toFixed(1)} C ${a.x.toFixed(1)} ${mid.toFixed(1)}, ${b.x.toFixed(1)} ${mid.toFixed(1)}, ${b.x.toFixed(1)} ${b.y.toFixed(1)}`);p.setAttribute('class',`campaign-link quasar-link ${cls}`);layer.appendChild(p)}
...
new MutationObserver(()=>{sync();schedule()}).observe(map,{subtree:true,attributes:true,attributeFilter:['hidden','class']});
sync();schedule();
})();
