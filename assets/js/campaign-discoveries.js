...
window.ARDUA_DISCOVERY_INDEX=Object.freeze(Object.fromEntries(PHENOMENA.map(({key,title,group,glyph,text})=>[key,Object.freeze({key,title,group,glyph,text})])));
...
function renderPhenomena(data,completed){
 ...
 for(const entry of PHENOMENA){if(!phenomenonOpen(entry,rewards,completed))continue;visible++;
  ...
  const b=document.createElement('button');b.type='button';b.className='discovery-card unlocked';b.dataset.discoveryKey=entry.key;b.dataset.discoveryText=entry.text;b.innerHTML=`<span class="discovery-glyph">${entry.glyph}</span><span><strong>${entry.title}</strong><small>${entry.group}</small></span>`;
