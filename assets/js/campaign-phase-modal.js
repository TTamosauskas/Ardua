...
function segmentFor(id){
 if(EARLY_SEGMENTS[id])return EARLY_SEGMENTS[id];const row=meta(id),gen=GEN?.generationOf?.(id),prefix=GEN_LABEL[gen];if(prefix&&row.branch)return`${prefix} - ${row.branch}`;return row.branch||prefix||'Campanha Cósmica';
}
...
map.addEventListener('click',e=>{
 if(bypassMapPhase)return;const node=e.target instanceof Element?e.target.closest('.phase-node[data-phase]'):null;if(!node)return;e.preventDefault();e.stopImmediatePropagation();openPreview(node.dataset.phase);
},true);

/* Engine-generated phase intros use the same compact information layout. */
function syncEngineModal(){
 if(!intro.classList.contains('show')||previewId)return;const id=C.getState?.().activeId;if(!id)return;renderModal(id,false);if(autoDismissEngine){autoDismissEngine=false;setTimeout(()=>engineStart.click(),0)}
}
...
sourceReady.then(()=>{if(previewId)renderModal(previewId,true);else syncEngineModal()});
})();
