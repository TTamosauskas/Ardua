...
ok(engine.includes('async function relocateNewbornCoreProduct(reactionProductIds=[])')&&engine.includes('explicitProducts=new Set(reactionProductIds||[])')&&engine.includes('const center=(byRing[0]||[])[0]')&&engine.includes('const ringOne=(byRing[1]||[])'),'produto de reação no núcleo central é deslocado para a primeira camada por identidade causal');
ok(engine.includes('const relocated=await relocateNewbornCoreProduct(reactionProductIds)')&&engine.includes('protectedPieceIds:protectedIds'),'produto deslocado do núcleo fica protegido do pulso gravitacional da mesma reação');
ok(engine.includes('reactionProductIds:[np.id]')&&engine.includes('forceBoardPulse:true,protectedPieceIds:protectedIds,reactionProductIds:[np.id]'),'fusão principal informa explicitamente o produto para retirada do núcleo central');
